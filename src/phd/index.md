...
И хотя интуиция верна, сравнение не вполне легитимно, поскольку задействует разные эпистемические допущения.
...
2. *Подмена объясняемого объясняющим.* Гуала, описывая пример с племенами, говорит, что сначала их разделяла река, и они могли пасти свои стада только на территории до реки. Когда река высохла, линия песка стала границей. Гуала объясняет это очевидностью (salience) истории игры: так всегда делали, поэтому будем делать и сейчас. Однако это сильное допущение.

Проблема в том, что пока есть река, игры «Ястреб-Голубь» и ситуации стратегического выбора нет — у обоих племён нет рациональных альтернатив своим территориям: переходить реку со стадами слишком рискованно. *Игра появляется как раз тогда, когда река высыхает*: физического ограничения больше нет, и можно заявить права на новую территорию. В этот момент ещё нет и не может быть конвенции «пастись, если нахожусь к югу» — есть только матрица «Ястреба-Голубя». Стратегия Буржуа лишь может появиться, но не гарантирована «историей игры», потому что игры как таковой не было. Более того, Буржуа-стратегия в данной игре не обязательно будет связана с положением племени относительно реки — устройство корреляции может быть любым.

3. *Институт как конвенция «второго уровня»*. Гуала имплицирует, что для существования института нужна уже существующая конвенция, решающая конфликт — например, «одно племя пасёт скот слева от высохшей реки, второе — справа». Институт Гуалы, следовательно — это равновесие в правилах уже существующей конвенции, которая, как говорит Гуала, возникает в силу очевидности истории игры. Если Буржуа-стратегия в игре животных — конвенция, то «пастись, если к югу от реки» — конвенция «второго уровня», посольку предполагает наличие готового правила. Подобная статичная картина отражает не суть института, а *результат идеальной работы института*, когда все конфликты уже решены.

4. *Подмена конститутивных правил регулятивными*. Если институт — конвенция «второго уровня», то правила для базовой конвенции, решающей конфликт откуда-то должны взяться. Базовая конвенция, решающая территориальный спор с помощью произвольного устройства корреляции, создаёт пространство игры «второго уровня», где появляется равновесие в правилах, которое Гуала и называет институтом. Однако Гуала не объясняет, откуда берутся эти правила. Вместо этого он говорит, что правила — это и есть устройства корреляции, что корректно только для случая, когда конфликт уже решён. Это создаёт напраяжение: Гуала, пытаясь схлопнуть различение конститутивных и регулитвных правил, сведя все к последним, говорит о двух разных конвенциях: конститутивные правила разрешают конфикт в базовой конвенции, а регулятивные регулируют поведение в игре «второго уровня». Схлопывая их, Гуала фактически подменяет базовую конвенцию конвенцией второго уровня, что нелегитимно и предполагает, что конфликт уже решён.
...
Поэтому стоит рассмотреть игру с племенами как игру с конфликтом, а не на координацию. Для этого её нужно сделать более похожей на классическую ЯГБ. В этой игре также есть два племени, однако после высыхания реки они решают оспорить территорию. В этой игре встречаются два пастуха, один из которых первым занял территорию, а второй пригнал стадо. У каждого пастуха есть три стратегии — драться, отступить или драться, если первым занял территорию, и отступить, если пригнал стадо.
...
\end{array}
$$
<figcaption class="caption">Территориальный спор пастухов</figcaption>
</div>

И хотя матрица не изменилась, изменился смысл: в этой игре есть только базовая конвенция — «драться, если занял территорию первым и отступать, если вторым». Это тоже правило вида «если…, то…», но оно не отсылает к уже существующей конвенции, а само ей становится. По форме это не отличается от института Гуалы — есть и правила, и коррелированное равновесие. Однако это не правила в онтологическом смысле, какими их хочет видеть Гуала — они не служат устройствами корреляции и не обладают нормативной и принудительной силой.

Достаточно ли такой конструкции для института? Гуала сказал бы, что нет, потому что здесь нет правил готовой конвенции, а если условные стратегии с подлинно экзогенной корреляцией.

Подобная конструкция чрезвычайно хрупка — нужно каждый раз драться, чтобы отстоять свой ресурс, и здесь я согласен с интуицией Гуалы: институт не просто минимизирует конфликты по поводу ресурса, как это делает Буржуа-стратегия в ЯГБ, а почти исключает их, превращая игру с конфликтом в игру на координацию. Однако если Гуала, чтобы показать это, тайком вводит дополнительный уровень конвенции, где конфликт уже решён, то как мы можем перейти от ситуации конфликта к ситуации чистой координации?
