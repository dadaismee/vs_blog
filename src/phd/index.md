...
В первой главе диссертации я описываю проблему, — как возможна принудительная сила социальных институтов как онтологический эффект разделяемых эпистемических конструкций (верований и ожиданий) — а также рассматриваю её ключевые решения — традицию социальной онтологии Джона Сёрла и его последователей [@searle1995; @searle2010; @tuomela2013; @gilbert1992; @bratman2022], традицию теоретико-игрового анализа социальных конвенций Дэвида Льюиса [@lewis1969; @vanderschraaf1995; @gintis2009a], институциональной экономики [ @acemogly2005; @aoki2011; @aoki2007; @north1990], а также синтез этих традиций в унифицированной социальной онтологии правил-в-равновесии Франческо Гуалы [@guala2015; @guala2016a] и его критику [@searle2015; @rabinowicz2018; @hedoin2021; @Vanderschraaf_2017].
...
<!--| Hawk | -1    | 2     | 0.5   |-->
<!--| Dove | 0     | 1     | 0.5   |-->
<!--| Bourgeois | -0.5  | 1.5   | 1.0   |-->

И хотя интуиция верна, само сравнение не вполне легитимно, поскольку задействует разные эпистемические допущения: в случае животных стратегии в матрице — это фенотипы, то есть разные доли популяции, а в случае людей — действия агентов. И хотя оба случая можно описать как КР, их интерпретация необходимо будет разной — пробабилстической в случае животных и эпистемической — в случае людей.

Чтобы показать это, я обращаюсь к Мейнарду Смиту [@maynardsmith1982], на которого ссылается Гуала. Мейнард Смит использует игру ЯГБ и вводит стратегию Буржуа, чтобы обосновать концепцию эволюционной стабильности (ЭСС) — когда профиль стратегий настолько стабилен, что не допускает новых стратегий-«мутантов». Допущения эволюционной и классической игры отличаются: стратегия Буржуа в эволюционной игре — не стратегия в смысле осознанного выбора агентов, а результат объективной популяционной динамики, в то время как в классической игре подобная условная стратегия доступна агентам для выбора.

По форме случай ЯГБ у Мейнарда-Смита, на который опирается Гуала для аргументации недостаточности равновесий, похож на КР Ауманна [@aumann1987]: есть источник публичного сигнала — асимметрия среды (роль «владелец» или «захватчик»), которая «видна» обоим агентам. Если один владеет территорией, то второй — нет. Согласно Ауманну — автору концепции КР, для КР агенты должны знать общее распределение вероятностей устройства корреляции. Источник вероятности может быть разным в зависимости от интерпретации: субъективная степень уверенности в определённом исходе (degree of belief), предполагающая байесовскую рациональность [@aumann1987] или же объективная случайная величина, которую в литературе называют Природой [@gintis2009a; @skyrms2014]. В примере территориального спора животных Гуалы агенты-животные «знают» распределение вероятностей, потому что *приватный и общий сигнал взаимоисключающие* — каждый агент либо «чужак», либо «владелец», и зная свой тип, агент легко понимает тип второго игрока.

Условная стратегия в примере с животными сочетает в себе свойства КР и ЭСС — рекомендует действие в зависимости от контекста и устойчива к мутантным эксплуатирующим стратегиям. При этом источник вероятности в КР здесь — объективный: Природа назначает каждому игроку тип — «владелец» или «захватчик» — с вероятностью ¹⁄₂, и оба игрока знают общее распределение вероятностей. Корреляция стратегий здесь *экзогенна*, то есть не встроена в структуру платежей: в каждом новом территориальном споре Природа заново назначает игрокам типы.

Чтобы утверждать недостаточность равновесий и необходимость правил для определения института, Гуале нужно показать эквивалентность даваемых им примеров: свойства игры с животными должны повторяться в примере в человеческими племенами. .*Буржуа-стратегия должна иметь экзогенный источник корреляции и быть ЭСС, а не только КР*.

<!--если животные достигают равновесий без правил, то наличие равновесия ничего не говорит о правилах.-->

Хотя Гуала утверждает, что коррелированные стратегии — это внешние правила (external rules), источник корреляции в них — не экзогенный, как в примере с животными. Стратегия «пасти скот, если находишься к югу от реки» уже предполагает распределение стратегий, основанное на произвольном правиле, а не объясняет его. Это правило могло бы быть другим и уже включает в себя произвольно выбранное устройство корреляции — положение относительно реки.

Гуала рассматривает повторяющуюся игру и говорит, что изначально устройством корреляции и фокальной точкой была река. Однако, как он утверждает вслед за Льюисом [@lewis1969], история игры постепенно сама становится фокальной точкой, что для Гуалы эквивалентно становлению правилом вида «если…, то…». Иначе говоря, если на этапе формирования института корреляция и может быть экзогенной (или природной), то на этапе работы института корреляция становится эндогенной, поскольку зависит от существующего произвольного правила, полученного из истории игры. Гуала не разводит эти этапы эксплицитно.

С этим связана основная проблема отношения правил и равновесий в теории Гуалы — правила как *ex ante* и как *ex post*: онтологические условия возможности равновесий и их репрезентации агентами как «рецепты действия». В первом случае без правил не может быть равновесий — как в случае с публичным объявлением границы, а во втором — правил не может быть равновесий, потому что первые «репрезентируют стратегии, которым нужно следовать в игре» [@hindriks2015a, 466].

Интересно, что этой проблемы можно было бы избежать, введя два типа правил, которые очень похожи на конститутивные и регулятивные — различение, которое Гуала пытается схлопнуть. Ведь правила как условия возможности равновесий и создают пространство игры, а правила как предписания регулируют работу этой игры[^hindriks].

[^hindriks]: В этом заключается основное отличие теории Гуалы от поздней теории Фрэнка Хиндрикса, названной им «правила-и-равновесия» [@hindriks2019]. Хиндрикс, вслед за Серлом, различает два типа правил: первые создают пространство игры наподобие конститутивныхх, а вторые регулируют игру.

Проблема здесь в том, что само правило «пасти скот, если к югу от реки» предполагает существование соглашения и *уже* является конвенцией в смысле Льюиса или Вандершраафа.

ОСТАНОВИЛСЯ ЗДЕСЬ



<!--Когда Гуала берёт буржуа как прототип любой институциональной стратегии, он делает методологически опасный ход: он обобщает модель, в которой институты не нужны, на случаи, где они как раз и являются предметом объяснения.-->

<!--Гуала выбирает третий путь: объявляет буржуа прототипом, но одновременно говорит, что равновесия недостаточно. Это логически нестабильно: если буржуа — прототип, то равновесие достаточно, если равновесие недостаточно, буржуа не может быть прототипом.-->

<!--Достаточно функциональной интерпретируемости δ: игроки видят последствия δ в одной игре и корректируют стратегии в другой.-->
***

<!---->
<!--В случае животных КР, представленное симметричными условными стратегиями, эволюционно стабильно (ЭСС и КР совпадают), однако в случае людей такой гарантии нет.-->
<!---->

В этом смысле распределение вероятностей стратегий простое. Это предельный случай, когда условная стратегия — ЭСС, однако нельзя сказать, что она КР в строгом (эпистемическом) смысле Ауманна — в ней нет субъективных ожиданий[^corr&ESS]. 

<!--Из подобного предельного случая ошибочно делать вывод о недостаточности равновесий для описания социальных институтов людей вообще. -->
...
Помимо этого, Мейнард-Смит отдельно указывает, что есть класс игр с социальными контрактами (social contract games), в которые могут играть *только люди*. В них игроки предпочитают общее благо, которое может идти вразрез с личными интересами [#insertSource]. Этого аргументация Гуалы не схватывает.

---

Гуала утверждает, что разница между социальным институтом у людей и «конвенцией» животных (вроде разрешения территориального спора) — в объёме сигналов для действия (scope of actionable signals): животные могут использовать только «генетически встроенные» устройства корреляции — очередность занятия территории, размер оппонента и т.д. Однако люди способны выбирать устройства корреляции произвольно, что для Гуалы равноценно способности одновременно следовать нескольким правилам. Опираясь на идеи Кима Стерельны [@sterelny2003], Гуала утверждает, что способность к «расцепленной» когнитивной репрезентации (decoupled representation), которая отделяет полученный сигнал от мгновенного поведенческого ответа, позволяет людям выбирать и создавать новые правила, которым они будут следовать.

В этом моменте Гуала приравнивает устройство корреляции и регулятивное правило, хотя это не следует из логики аргумента. Чтобы утверждать недостаточность равновесия и необходимость правил, Гуале нужно *показать* разницу устройств корреляции в обоих случаях вместо её постулирования.

У животных сигналы в ЯГБ общие, потому что вероятных «приватных» сигналов всего два (роль «владелец» или «захватчик»), они взаимоисключающие, и устройство корреляции не посылает каждому агенту истинно приватный сигнал, недоступный другим, как в КР [@aumann1987; @vanderschraaf1995]. Развёрнутая форма игры из примеров Гуалы позволяет увидеть это более явно:

![Игра с полной информацией](./assets/hdb-extended_animals.png)

![Игра с неполной информацией](./assets/hdb-extended_humans.png)

> Кажется, я неправильно понял Гуалу: для него равновесие и правило — это онтологически одно и то же в зависимости от перспективы. Для юзера это правила «если, то», которые описывают стратегию при получении сигнала, для наблюдателя это КР
...
Хотя в нормальной форме матрицы этих игр идентичны, в развёрнутой форме они имеют разную *информационную структуру*. В случае животных сигнал отражает состояние мира индексично — буквально «означает» тип «чужак» или «владелец», в то время как у людей получатель не может однозначно вывести состояние мира из сигнала, и возникает неопределённость.

ОСТАНОВИЛСЯ ЗДЕСЬ

<!--обоснование ввода развернутой формы-->
<!---->
<!--### Критический анализ Bourgeois в контексте CE и ESS-->
<!---->
<!--1. **Ограничения CE-анализа (Гуала, 2016)**-->
<!---->
<!--   * Гуала рассматривает Bourgeois как **коррелированное равновесие** в нормальной форме, где сигналы природы (тип Owner/Intruder) уже встроены в стратегию.-->
<!--   * В этом подходе **неявно предполагается**, что все игроки следуют коррелированным рекомендациям, но **не отражается, какие действия реально доступны игроку**.-->
<!---->
<!--2. **Развёртка игры**-->
<!---->
<!--   * Вводим **Nature**, назначающую тип игрока, и отображаем все ветки действий в зависимости от типа (Owner / Intruder).-->
<!--   * Это позволяет явно увидеть **scope actionable signals**: какие стратегии может выбрать игрок, зная свой тип.-->
<!---->
<!--3. **Эпистемические различия между животными и людьми**-->
<!---->
<!--   * **Животные**: тип известен каждому игроку, сигналы фиксированы, полная информация → Bourgeois **устойчива к редким отклонениям** и является ESS.-->
<!--   * **Люди**: сигналы могут быть фальшивыми (cheap talk / блеф), тип скрыт → Bourgeois **не является ESS**, хотя CE сохраняется.-->
<!---->
<!--4. **Вывод о институтах**-->
<!---->
<!--   * Корреляция стратегии (CE) **не гарантирует устойчивость института**.-->
<!--   * Для оценки институциональной устойчивости необходимо учитывать **информацию о типах и возможность отклонений**, т.е. проверять **эволюционную устойчивость стратегии**.-->
<!--Подобную разницу Гуала интуитивно схватывает как «разницу в объеме действенных сигналов» (scope of actionable signals), однако не формализует её, что приводит к ошибочным выводам о роли регулятивного правила в социальных институтах и роли репрезентации в их существовании и поддержании.-->
...
Подобная ситуация требует механизма стабилизации на уровне платежей в матрице игры. Однако решение Гуалы предполагает идеальное соответствие посылаемого сигнала типу игрока — даже в случае людей — и исключает неопределенность. Из-за этого постулирование регулятивного правила как коррелятора стратегий у людей в отличие от животных выглядит надуманным, поскольку не связано с разницей объема информации в сравниваемых играх.
...
Иначе говоря, решение Гуалы *предполагает*, а не объясняет мотивацию следовать регулятивному правилу, с помощью которого агенты репрезентируют равновесие, а просто постулирует её.

Чтобы разрешить это противоречие, нужно решить проблему блефа: показать условия эволюционной стабильности в игре Ястреб-Голубь-Буржуа с неполной информацией (то есть вероятностью блефа). Подобные условия эволюционной стабильности дадут ответ на минимальную онтологическую архитектуру социального института, и прояснят отношение между равновесием и регулятивным правилом.
...
Во второй главе исследования я описываю два направления решения проблемы источника принудительной силы в унифицированной социальной онтологии:
...
### Глава 3. Корреляция стратегий как компенсация энтропии сигналов
...
Стратегия Буржуа в ЯГБ — эволюционно стабильна, если типы наблюдаемы игроками, а это эквивалентно одиночным информационным множествам в игре в развёрнутой форме. Если $h \in H$ — конечная последовательность ходов игры $h = (a_1, a_2,\dots, a_k)$, то у в ЯГБ в примере Гуалы с животными они будут:
$$
\begin{gathered}
h_1 = \{\theta_1, m_1\} \\
h_2 = \{\theta_2, m_2\} \\
...
Тип отправителя неразличим для получателя сигнала — он видит один и тот же сигнал «я владелец» и когда отправитель действительно «владелец», и когда он «чужак». Это создаёт неопределённость и лазейку для эксплуатации получателя. 

Нетривиальные информационные множества позволяют ввести понятие уловной энтропии сигнала — количество информации, недостающее для точного определения получателем типа отправителя по его сигналу:
...
Это означает, что *эволюционно стабильное коррелированное равновесие не может существовать в среде с положительной энтропией без модификатора платежей*. Введение $\delta$ делает стратегию Буржуа устойчивой на уровне платежа, что «переводит» статистику популяции в стратегию, доступную агенту для выбора. Онтологически это означает, что санкция $\delta$, а не равновесие само по себе — необходимый элемент социального института, отличиющий простую конвенцию от института.
...
Коррелированная стратегия Буржуа будет эволюционно стабильной, поскольку симметрична, несмотря на свою условность.
...
Чем больше неопределенность относительно сигнала о типе игрока, тем сильнее санкция за эксплуатацию этой неопределенности одним из игроков. Онтологически это означает, что корреляция стратегий стабилизируется не репрезентацией рагентами равновесия с помощью регулятивных правил, как считает Гуала [@guala2016a], и не общим знанием о социальной норме как коррелирующем устройстве, как считает Гинтис [@gintis2009a], а с помощью объективной санкции за факт эксплуатации неопределенности.
