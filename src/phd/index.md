...
\end{array}
$$
<figcaption class="caption">Случай территориального спора животных: когда оба игрока выбирают <span style="color: red">условную стратегию</span> (играть Ястреба, если владеешь территорией, и Голубя, если вторгаешься на чужую), каждый получает равный выигрыш.</figcaption>
</div>
...
И хотя интуиция верна, само сравнение не вполне легитимно, поскольку задействует разные эпистемические допущения.

1. *Игра животных — с конфликтом, а игра племён — на координацию (конфликт уже решён)*. Описывая пример с бабочками, оспаривающими территорию, Гуала говорит, что игроки вступают в конфликт. Буржуа-стратегия, согласно Мейнарду-Смиту, на которого опирается Гуала, минимизирует встречи «Ястреб-Ястреб», играя «Голубя» при встрече с «Ястребом». Условность Буржуа-стратегии направлена на избегание конфликта. В то же время стратегии в игре с племенами предполагают, что *конфликт уже решён* и территория не оспаривается: игра предполагает существование конвенции о разделении территории, и игрокам остаётся лишь следовать уже существующему правилу — «пасти скот, если к югу от реки».

2. *Нелегитимный перенос экзогенности корреляции*. В случае бабочек корреляция стратегий экзогенна — опирается на природу (кто первый занял место), в случае племён — эндогенна и опирается на существующую конвенцию (как мы решили раньше). Устройство корреляции в первом случае объективное, а во втором — интерсубъективное.

3. *Подмена объясняемого объясняющим.* Гуала описывает пример с племенами, говоря, что сначала их разделяла река, и племена могли пасти свои стада только на территории до реки. Когда река высохла, линия песка стала границей. Гуала объясняет это очевидностью (salience) истории игры: так всегда делали, поэтому будем делать и сейчас. Однако это сильное допущение.

Пока есть река, игры «Ястреб-Голубь» и ситуации стратегического выбора нет, потому что у обоих племён нет рациональных альтернатив своим территориям: переходить реку со стадами слишком рискованно. *Игра появляется как раз тогда, когда река высыхает*: физического ограничения больше нет, и можно заявить права на новую территорию. В этот момент ещё нет и не может быть конвенции «пастись, если нахожусь к югу» — есть только матрица «Ястреба-Голубя», а стратегия Буржуа лишь может появиться, но не гарантирована «историей игры», потому что игры как таковой не было. Более того, Буржуа-стратегия в данной игре не обязательно будет связана с положением племени относительно реки — устройство корреляции может быть любым. 

4. *Институт как конвенция «второго уровня»*. Гуала имплицирует, что для существования института нужна уже существующая конвенция, решающая конфликт — например, «одно племя пасёт скот слева от высохшей реки, второе — справа». Институт Гуалы, следовательно — это равновесие в правилах уже существующей конвенции, которая, как говорит Гуала, возникает в силу очевидности истории игры. Если Буржуа-стратегия в игре животных — конвенция, то «пастись, если к югу от реки» — конвенция «второго уровня». Подобная статичная картина отражает не суть института, а *результат идеальной работы*, когда конфликты уже решены.

5. Проблема подмены конститутивных правил регулятивными. Если институт — конвенция «второго уровня», то правила для базовой конвенции, решающей конфликт откуда-то должны взяться, чтобы создать пространство для игры «второго уровня», где появляется равновесие в правилах. Гуала не объясняет, откуда берутся эти правила. Вместо этого он говорит, что правила — это и есть устройства корреляции, что корректно только для случая, когда конфликт уже решён. Это создаёт напраяжение: Гуала, пытаясь схлопнуть различение конститутивных и регулитвных правил, сведя все к последним, говорит о двух разных конвенциях: конститутивные правила разрешают конфикт в базовой конвенции, а регулятивные регулируют поведение в игре с правилами. Схлорывая их, Гуала фактически подменяет базовую конвенцию конвенцией второго уровня, что нелегитимно и предполагает, что конфликт уже решён.


в случае животных стратегии в матрице — это *фенотипы*, то есть разные доли популяции, а в случае людей — действия агентов. И хотя оба случая можно описать как КР, их интерпретация будет разной — пробабилстической в случае животных и эпистемической — в случае людей.
...
По форме случай ЯГБ у Мейнарда-Смита, на который опирается Гуала для аргументации недостаточности равновесий, похож на КР Ауманна [@aumann1987]: есть источник публичного сигнала — асимметрия среды (роль «владелец» или «захватчик»), которая «видна» обоим агентам. Если один владеет территорией, то второй — нет. Согласно Ауманну — автору концепции КР, для КР агенты должны знать общее распределение вероятностей устройства корреляции. Это требование выполняется в этом примере. Однако источник вероятности может быть разным в зависимости от интерпретации: субъективная степень уверенности в определённом исходе (degree of belief), предполагающая байесовскую рациональность [@aumann1987] или же объективная случайная величина, которую в литературе называют Природой [@gintis2009a; @skyrms2014]. В примере территориального спора животных Гуалы агенты-животные «знают» распределение вероятностей, потому что *приватный и общий сигнал взаимоисключающи* — каждый агент либо «чужак», либо «владелец», и зная свой тип, агент легко понимает тип второго игрока.
...
Этой проблемы можно было бы избежать, введя два типа правил, которые очень похожи на конститутивные и регулятивные — различение, которое Гуала пытается схлопнуть. Ведь правила как условия возможности равновесий в примере Гуалы с племенами и создают пространство игры, а правила как предписания регулируют её работу[^hindriks].
...
Постулируя разницу устройств корреляции, Гуала по-прежнему описывает их как внешние по отношению к игре и в случае животных, и в случае людей, однако, как мы показали выше, это неверно. Само правило, коррелирующее стратегии игроков, уже является произвольной конвенцией, эндогенной игре *в силу её истории*.
...
### Эволюционная стабильность и неполнота информации в примерах Гуалы
...
### Глава 3. Эволюционная стабильность условной стратегии в игре с неполной информацией: формальная модель
