...
- Гуала сравнивает разные игры: 
  ...
  - игру с конфликтом с игрой на координацию, не изменяя матрицы:
    ...
    - Ключевой момент: Фраза «продолжают пасти» содержит готовую конвенцию. Но она взялась? В момент, когда река высохла, перед племенами встала точно такая же игра с конфликтом, как у животных (ЯГБ за новый ресурс). Гуала пропускает этот момент и начинает анализ с того, что конвенция уже существует.
- Вывод: Гуала не моделирует возникновение института. Он моделирует воспроизводство уже существующего института в слегка изменившихся условиях (река исчезла, но «правило» осталось). Его модель объясняет не «почему есть граница», а «почему сохраняется инерция уже существующей конвенции о границе, если физический маркер исчез». Это другая, более простая проблема.

#### 2.1.2. Логическая структура подмены: институт как *explanans* vs. институт как *explanandum*
...
- Итог: Он строит теорию, где институт (как готовая конвенция) является предпосылкой для объяснения института (как устойчивого равновесия). Это логический круг (petitio principii). Он объясняет институт через самого себя.
- Это фатально потому что весь пафос Гуалы — показать, как **правила и равновесия совместно порождают институты**. Но в его ключевом примере правило («паси на своей стороне») **не порождается** игрой — оно **дано ей изначально** как часть истории. Таким образом, его синтез не демонстрирует **генезис** институциональной реальности из до-институционального состояния. Он демонстрирует лишь её **инерцию**.
