...
---
Наша модель не утверждает, что каждый институт буквально реализует игру “ястреб–голубь”, и не сводит все социальные порядки к борьбе за ресурс в узком биологическом смысле. Она использует H–D–B–L как **структурный шаблон**: в доинституциональном состоянии есть агрессия и уход (H и D), ролевая условная стратегия B (играть по роли, если я “здесь и сейчас владелец/первый/уполномоченный”), и оппортунистический блеф L (притворяться B, не будучи им). Институт затем описывается как структура игры и параметры санкций \((\delta,r)\), при которых B становится выгоднее L и вытесняет доинституциональные H,D на фон. [plato.stanford](https://plato.stanford.edu/archives/sum2025/entries/game-theory/)

В этом смысле модель применима не только к классическому ЯГБ, а ко всяким ситуациям, где есть: (i) конфликт за ценность \(V\) (ресурс, доступ, статус), (ii) **наблюдаемая асимметрия ролей**, одинаково читаемая игроками (первый/второй, владелец/интрудер, резидент/гость, отправитель/получатель), и (iii) **неполная информация о типе**, позволяющая блеф: наблюдатель видит заявку на роль, но не знает, честный это B или оппортунист L. В таких играх — будь то налоги, лицензирование, доступ к очереди, сигнальные рынки (“я квалифицирован”, “товар качественный”) — можно построить матрицу выигрышей с ролями и стратегиями “по роли / против роли / блеф / уход” и затем наложить наш санкционный слой: сила санкции \(\delta\), надёжность её применения \(r\), и пороговое условие вида \(r\delta > f(\text{выигрыши})\), при котором буржуазное поведение становится эволюционно устойчивым. [sites.socsci.uci](https://sites.socsci.uci.edu/~bskyrms/bio/papers/salience_symmetry-breaking_convention.pdf)

Модель, таким образом, **не говорит, что институт = ЯГБ**, но говорит, что **институциональная динамика во всех случаях, где есть борьба за ценность, ролевая структура и блеф, HDBL‑подобна**. Доинституциональный фон или момент распада всегда включает в себя опции “нажимать силой” (H) и “отходить/подчиняться” (D); институт вводит ролевую стратегию B, а оппортунистический тип L начинает эксплуатировать неполную информацию. Санкционный канал \((\delta,r)\) задаёт, будет ли B удерживать порядок, или же L и H вытеснят его. При достаточном \(r\delta\) репликаторная динамика (или обучение) вытесняет отклонения, закрепляя буржуазный режим; при недостаточном \(r\delta\) институт остаётся “на бумаге”, а поведение откатывается к доинституциональным комбинациям H+L.
Чтобы модель делала объяснительную работу, нужны несколько ключевых допущений. Во‑первых, **есть микроконфликт за ценность**: игроки реально предпочитают разные исходы, а не просто координируются ради удобства. Во‑вторых, есть **наблюдаемая асимметрия ролей**: правила типа “первый пришедший”, “формальный владелец”, “старший по статусу” доступны и одинаково интерпретируются обоими агентами, что позволяет определять условную стратегию B (“если я в роли R1, действую так”). В‑третьих, есть **неполная информация о типе**: нельзя гарантированно отличить честного носителя роли от блефующего — есть документы, сигналы и формы поведения, которые можно подделать или симулировать. В‑четвёртых, существует **канал санкций** с параметрами \(\delta\) (издержка наказания для нарушителя) и \(r\) (доля нарушений, которые реально доходят до санкции, зависящая от числа игр/контекстов и инвестиций в контроль). И наконец, поведение адаптивно: стратегии, которые дают выше ожидаемый payoff при данных \((V,\delta,r)\), со временем распространяются (эволюция или обучение). [plato.stanford](https://plato.stanford.edu/archives/sum2025/entries/game-theory/)

Границы модели появляются там, где эти допущения ломаются. В **чистых координационных нормах** без оппортунизма и внешних санкций (сторона движения, формат разъезда) нет ни типа L, ни канала \((\delta,r)\): здесь лучше работают модели чистой эволюции конвенций, а не HDBL. В нормах идентичности и выражения (ритуалы, вкусы, стиль), где нет значимого конфликта за ресурс и блеф не даёт односторонних выгод, HDBL‑структура тоже слишком груба. Наконец, если нет **общепринятой и наблюдаемой асимметрии ролей** (игроки не согласны, кто “владелец”, или роль не читается одинаково), то сама релевая стратегия B плохо определена, и модель не стартует. В этих случаях она не должна претендовать на объяснение. [sites.socsci.uci](https://sites.socsci.uci.edu/~bskyrms/bio/papers/salience_symmetry-breaking_convention.pdf)

В сумме, модель утверждает следующее: везде, где есть конфликт за ценность, ролевая асимметрия, оппортунизм и санкции, институт можно анализировать как HDBL‑тип структуру — как динамику между ролевым поведением и блефом под действием канала санкций, с чётко определённой пороговой областью параметров, в которой институт устойчив или рушится. За пределами этих условий она не универсальна и должна сочетаться с другими типами моделей. [users.ox.ac](https://users.ox.ac.uk/~grafen/cv/bourgeois.pdf)
---

Основное требование нашей модели — наличие *наблюдаемой асимметрии ролей*, которую оба участника могут одинаково интерпретировать. Такая асимметрия может быть очень простой (например, «пришёл первым», «занимает более высокую позицию в иерархии») или сложной, но важно, что она доступна обоим игрокам и они согласны, кто находится «в сильной» и «в слабой» роли в данном взаимодействии. Как показывают Скирмс и коллеги, появление условных стратегий требует *любой* асимметрии в окружающей среде, которая бы коррелировала действия игроков [@herrmann2021; @skyrms1998]. Она может исходно быть нейтральной (кто пришёл первым, как случайно распределены цвета сигналов и т.п.), но как только стратегии начинают на неё условиться, она превращается в основу ролевых норм. Такая асимметрия может быть встроена в самые разные типы игр. В играх распределения и конфликтов вроде ЯГБ она задаёт роли владельца/захватчика. В координационных играх и «битве полов» — роли сторон. В играх обмена и сигнализирования — статусы отправителя и получателя, продавца и покупателя. Ключевое свойство: как только в игре появляется устойчиво распознаваемая асимметрия ролей, агенты могут эволюционно выработать условные стратегии, которые выбирают разные действия в разных ролях, и именно это открывает место для института.

Второе требование — *неполнота информации о типе игрока*. Это означает, что даже если асимметрия наблюдаемая, её можно «подделать» и эксплуатировать неопределённость, введя оппонента в заблуждение. При полной информации — когда оба игрока точно знают тип оппонента, это становится игрой на чистую координацию и внешняя санкция, принуждающая к определённому поведению, не нужна.

<!--- сказать в каких играх возможна неполная информация — todo-->

Третье требование — *наличие множества игр, в пространстве или времени*. Это требование не к конкретной игре, а к объяснительной функции самой модели. Модель имеет смысл только при положении множества игр, поскольку без него она «не видит» большинство важных для нас аспектов института. Вместо этого она в таком случае объясняет только локальное равновесное поведение в одной игре, что, возможно, послужило бы ответом на критику Вандершраафом теории Гуалы и Хиндрикса [@Vanderschraaf_2017], однако этого недостаточно для объяснения института, по нашему мнению.

Рассмотрим игру… 

<!--- выбрать игру с неполной инфой и асимметрией ролей и рассмотреть ее так же как ЯГБ — todo-->


### 3.4. Формальное определение: институт, протоинститут, конвенция
Определив границы модели, мы можем дать определения того, что схватывает наша модель. 

<!--- узнать как встаивть латеховское ОПРЕДЕЛЕНИЕ — todo-->
...
Таким, образом, в этой главе мы построили параметрическую модель социального института. Основные параметры — размер санкции за блеф $\delta$ и взаимная информация между применением санкции в разных играх $I(E_i, E_j)$. Также мы рассмотрели условия применимости нашей модели: наличие асимметрии ролей, неполноты информации и множества идентичных игр. Это позволило нам отделить института от протоинститута и конвенции.
