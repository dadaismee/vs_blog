...
Рассмотрим конечную игру с медиатором. Игроки \(N = \{1,\dots,n\}\). Для каждого игрока \(i\) заданы:

- множество действий \(A_i\);  
- множество сигналов (ролевых рекомендаций) \(S_i\).

Обозначим \(A = \prod_i A_i\), \(S = \prod_i S_i\). Базовые выигрыши задаёт функция \(u_i: A \to \mathbb R\).

Медиатор выбирает профиль сигналов \(s = (s_1,\dots,s_n) \in S\) по распределению \(\mu \in \Delta(S)\) и сообщают каждому игроку \(i\) только его компонент \(s_i\). Стратегия игрока \(i\) — это функция 

\[
\sigma_i: S_i \to A_i,
\]

которая каждому сигналу \(s_i\) ставит в соответствие действие \(\sigma_i(s_i)\). [en.wikipedia](https://en.wikipedia.org/wiki/Correlated_equilibrium)

У Вандершраафа конвенция — это распределение \(\mu\) и профиль стратегий \((\sigma_i)_i\), образующие коррелированное равновесие: для любого игрока \(i\), сигнала \(s_i\) с \(\mu(s_i)>0\) и любого возможного отклонения \(a_i' \in A_i\) выполняется условие

\[
\mathbb E\bigl[u_i(\sigma_i(s_i),\sigma_{-i}(s_{-i})) \mid s_i\bigr]
\;\ge\;
\mathbb E\bigl[u_i(a_i',\sigma_{-i}(s_{-i})) \mid s_i\bigr].
\]

Теперь я добавляю санкционный канал. Институт, помимо коррелирующего распределения \(\mu\), задаёт:

- \(r \in [0,1]\) — надёжность санкций;  
- \(\delta > 0\) — размер санкции.

Каждому сигналу \(s_i\) сопоставлено ролевое предписание \(a_i^{\text{rec}}(s_i) \in A_i\) (“что предписывает правило” при данном сигнале). В простейшем случае \(S_i = A_i\), и тогда \(a_i^{\text{rec}}(s_i)=s_i\).

Определим индикатор девиации:
...
Институциональный ожидаемый выигрыш игрока \(i\) при сигналах \(s\) и действиях \(a\) задаётся как
...
Если игрок следует ролевому предписанию, \(D_i=0\) и штраф отсутствует; если он отклоняется, его payoff уменьшается на ожидаемую величину \(r\delta\). Величина \(r\) интерпретируется как вероятность того, что нарушение будет обнаружено и повлечёт санкцию (условная частота “нарушение → санкция” в большом семействе эпизодов). [arxiv](https://arxiv.org/html/2402.09556v1)
...
Если \(a_i' = a_i^{\text{rec}}(s_i)\), то \(D_i=0\), и условие сводится к обычному условию конвенции Вандершраафа (коррелированного равновесия). Если \(a_i' \neq a_i^{\text{rec}}(s_i)\), появляется отрицательный слагаемый \(-r\delta\), отражающий ожидаемый штраф за блеф или оппортунистическое отклонение.
...
Институциональное условие невыгодности блефа тогда принимает простую форму:

\[
r\delta \;\ge\; \Delta u_i(s_i;a_i',a_i^{\text{rec}}(s_i)).
\]

Эта формула показывает, почему нельзя “превратить любую игру в институт”, просто добавив штраф \(\delta\): во‑первых, санкция должна быть привязана именно к отклонениям от ролевых рекомендаций, а не вычитаться из всех платежей; во‑вторых, заданные параметры \((r,\delta)\) должны быть достаточно сильны, чтобы в **конкретной игре** и при **конкретной конвенции** перекрыть премию от оппортунизма. В разных играх \(\Delta u_i\) будет разной функцией от структурных параметров (например, от \(V\) и \(C\) в ястреб–голубь–буржуа), и, следовательно, пороговые значения \((r,\delta)\) будут различаться.
...
### 3.2 Реализация санкций и институт как область параметров

<!--- санкция применяется **не всегда**, поэтому $EU(g_i) = u(g_i,f_{-i}) - r\delta$]-->
