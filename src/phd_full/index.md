...
При $\delta = \frac{V}{4} + \epsilon = 50/4 + 0.01  = 12.51$ профиль с условными стратегиями $(B, B)$ будет ЭСС и противостоит внедрению блефующей стратегии. Минимальное нужное значение — именно $12.51$, поскольку при $12.5$ платёж Блефующего ($25$) будет равным платежу Буржуа ($25$), чего недостаточно для ЭСС — одна стратегия должна давать строго больший выигрыш: $f(I, I) > f(J, I)$ [@maynardsmith1982]. Если снизить значение $\delta$, равновесие разрушится и перейдёт к флуктуации — мерцанию между разными исходами.

Однако это так только для статической проверки ЭСС. В более реалистичных — динамических — условиях — этого будет недостаточно. Наглядно показать это можно с помощью *репликаторной динамики* — для более высоких и более низких значениях $\delta$. Пусть $x_i$ — это доля популяции, использующая стратегию $i$ (например, $B$), $f_i$ — ожидаемый выигрыш стратегии $i$. Уравнение репликатора:
...
где $\bar{f} = \sum_j x_j f_j$ — средний ожидаемый выигрыш по популяции. При наличии четырёх стратегий с равными изначальными долями (0.25) доли в популяции будут:

$$f_B = 0.25 \cdot (-12.5) + 0.25 \cdot (37.5) + \cdot (25) + \cdot (12.5) = $$

0.25*(-12.5)+0.25*(37.5)+0.25*25+0.25*12.5 = 15.625
0.25*0+0.25*50+0.25*24.99+0.25*(-25) = 12.4975

ОСТАНОВИЛСЯ ЗДЕСЬ

При $\delta = 12.51$ условная стратегия Буржуа стабильно занимает большую долю популяции:

<!--мб сделать 10000 экспериментов и показать или монтекарло, а не один запуск? — todo-->

При $\delta = 10$ условная стратегия Буржуа …
<!--сделать эксперименты для d = 10 — todo-->

При $\delta = 5$ условная стратегия Буржуа …
<!--сделать эксперименты для d = 5 — todo-->
