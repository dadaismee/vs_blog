...
Другими словами, Гуала и Хиндрикс, следуя Вандершраафу, видят институт как конвенцию — строгое КР с общим знанием. Они фактически допускают, что *правило уже обладает свойствами строго КР* — отклонение невыгодно и санкции не нужны. В их примере с выпасом скота это условие выполняется только потому, что они неявно предполагают информационную структуру, в которой игроки молчаливо согласились использовать определённое устройство корреляции, тем самым исключив возможный конфликт за территорию и блеф. В реалистичной структуре, где устройство корреляции не появляется автоматически, правило $f$ перестаёт гарантировать реализацию строгого КР. Возникают состояния, где следование правилу приводит к конфликту, а отклонение — к выигрышу за счет другого.

Онтологически *институт не может быть конвенцией* (строгим КР с общим знанием), поскольку конвенция предполагает полную прозрачность сигналов, общее знание и полное отсутствие даже гипотетических причин для отклонения: единолично отклоняться от предписанного поведения невыгодно *в любом случае*. Это сильная и требовательная концепция социального порядка, описывающая ситуацию, где *принуждение не нужно*: структура мотивации и информации уже настолько выгодная, что не требует механизмов возвращения отклонений в равновесное состояние. 
...
В этой главе мы рассмотрим формальные условия устойчивости социального института — при каких условиях КР реализуется в игре с неполной информацией, а также формально покажем разницу института и протоинститута. Это позволит подготовить почву для более нюансированной унифицированной социальной онтологии, совмещающей равновесия, регулятивные и конститутивные правила.

<!--[? что и как мы будем моделировать]-->
Как мы выяснили в прошлой главе, КР недостаточно для стабильности института в случае неполной информации, и нужна санкция в виде $\delta$-параметра, вычитаемого из платежа Блефующего.

ОСТАНОВИЛСЯ ЗДЕСЬ

- при блефе есть стимул отклонения
- нужна санкция
- но санкция не гарантирована и вероятностная
- вероятностная санкция уравновешивает премию от оппортунизма 
  - r — не параметр игры, а механизм
- институт — *механизм*, обеспечивающий rδ
  - численно rδ — порог → картинка
  - институт стабилен в пространстве параметров
  - пример с ЯГБ
  - пример с охотой на оленя
- институт как сеть игр: эмпирическая природа r
  - сеть игр
  - эмпирическая частота
  - институт как система игр, а не одна игра
  - зачем? эмпирическая измеримость
- формальные определения
  - институт как $(\mu, \delta, r)$
  - 
