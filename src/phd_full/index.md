...
Следуя Бергеману и Моррису [@bergemann2016], зададим игру с неполной информацией $(G,S)$.  Базовая игра $G$ включает конечное множество игроков $i=1,\dots,I$, множества действий $A_i$, множество состояний $\Theta$ и общую априорную веру $\psi \in \Delta(\Theta)$.  Полезности игроков описываются функциями $u_i : A \times \Theta \to \mathbb{R}$, где $A = \prod_i A_i$.  Информационная структура $S = ((T_i)_i,\pi)$ задаёт множества типов $T_i$ и распределение типов $\pi : \Theta \to \Delta(T)$, $T = \prod_i T_i$.
...
Чтобы определить БКР, Бергеман и Моррис определяют «повиновение» (obedience) — условие для правила принятия решения $\sigma$. Они называют правило $\sigma$ «повинующимся» для $(G,S)$, если для каждого игрока $i$, каждого типа $t_i \in T_i$ и каждого рекомендованного ему действия $a_i \in A_i$ выполняется неравенство
$$
\sum_{a_{-i},t_{-i},\theta}
u_i((a_i,a_{-i}),\theta)\,
...
В сжатом виде это неравенство выглядит как 

$$
\mathbb E\big[u_i(a_i,\theta)\mid t_i,a_i\big]
\;\ge\;
\mathbb E\big[u_i(a_i',\theta)\mid t_i,a_i\big].
$$
...
- в плохом состоянии он теряет вложенное;
- в хорошем зарабатывает прибыль.
...
Это и есть содержательный смысл условия «повиновения» — если инвестор с определённым сигналом получает рекомендацию «инвестируй», то при своих апостериорных верованиях он должен считать инвестицию для себя не хуже отказа. А если он получает рекомендацию «не инвестируй», эта рекомендация тоже не должна идти в разрез с его апостериорными оценками.

Иначе говоря, для каждого типа инвестора (для каждого варианта сигнала) рекомендация дизайнера должна быть *самоподдерживающейся*: после обновления верований у агента не возникает мотива её нарушить.
...
<!--Другими словами, информационный дизайнер лишь управляет верованиями агентов и не влияет на материальные выигрыши агентов.-->
...
Во‑вторых, БКР не различает институциональные и другие последствия. В функции выигрыша любой элемент смешан с технологическими и другими эффектами: сломалась машина, упала цена или начал испытывать стыд. Всё это — числа в матрице, обеспечивающие устойчивость. Для онтологии института, напротив, нужно выделить те компоненты, которые являются социально‑созданными ответами на соблюдение или нарушение правила.

В‑третьих, без дополнительной параметризации БКР не даёт удобного способа говорить о «силе» одного и того же института. Оно либо есть, либо нет. Между тем в эмпирической и философской литературе именно градуальность — сильные и слабые институты, жёсткие и мягкие нормы и санкции — постоянно выходит на первый план [@ostrom1990; @bicchieri2005; @hindriks2022].

ОСТАНОВИЛСЯ ЗДЕСЬ

Чтобы расширить и обобщить теории ПвР и ПиР и связать их с БКР, нужно выполнить ещё один шаг: добавить в структуру выигрышей санкцию за нарушение правила, структурную необходимость которой мы показали в прошлой главе. Тогда стандартное условие «повиновения» можно переписать в более онтологически содержательной форме. 

В терминах БКР оно говорит:

> при данных сигналах ожидаемый выигрыш от следования рекомендованному действию не меньше выигрыша от отклонения.

В терминах институциональной интерпретации то же самое условие можно разложить на:

> ожидаемые «обычные» выгоды + ожидаемый институциональный эффект (вероятность санкционного исхода, помноженная на \(\delta\)) не меньше совокупной выгоды от нарушения правила.

Тогда:

- *информационная структура БКР* задаёт, как разные типы агентов оценивают вероятность санкционного исхода (через апостериорные верования по состояниям);
- *параметр \(\delta\)* выражает интенсивность самих санкций или институциональных последствий;
- *правило* совместно с санкцией определяют, какие действия считаются соблюдением, а какие — нарушением.
...
<!--[? где дельта в БКР?]-->
Чтобы описать институт с санкциями, нужно добавить модификатор платежей. Пусть задан параметр $\delta_i > 0$, интерпретируемый как размер санкции для игрока $i$ при отклонении от нормы. Тогда модифицированная полезность $\tilde u_i$ определяется как
$$
\tilde u_i(a,\theta)
=
u_i(a,\theta)
-\delta_i \cdot \mathbf{1}\{\text{в профиле } a \text{ игрок } i \text{ отклоняется от рекомендации }\sigmaкак
$$
Институт с санкциями реализует норму $\sigma$, если $\sigma$ является БКР игры $(G,S)$ с полезностями $\tilde u_i$, то есть условие $(OB)$ выполняется при замене $u_i$ на $\tilde u_i$. 

Разность правой и левой частей $(OB)$ при фиксированных $(i,t_i,a_i,a_i')$ можно разложить на две составляющие. Первая — это *структурная премия за девиацию* в игре без санкций:
$$
\Delta_i(t_i,a_i,a_i')
=
\mathbb E\big[u_i(a_i',\theta)\mid t_i,a_i\big]
...
Вторая — *ожидаемый вклад санкции*, возникающий из замены $u_i$ на $\tilde u_i$. Если обозначить через $\hat r_i(t_i,a_i,a_i')$ субъективную (байесовскую) вероятность того, что девиация игрока $i$ будет наказана, то ожидаемый штраф при отклонении равен $\hat r_i(t_i,a_i,a_i')\,\delta_i$. В таком параметрическом представлении обобщённое условие obedience для данной тройки $(t_i,a_i,a_i')$ можно записать в свернутом виде
$$
\hat r_i(t_i,a_i,a_i')\,\delta_i
\;\ge\;
\Delta_i(t_i,a_i,a_i').
\tag{OB}
$$

Неравенство $(OB)$ — это просто компактная параметрическая форма стандартного условия $(OB)$; оно ничего не добавляет к определению БКР, но удобно для интерпретации. В частных примерах, таких как 2×2‑игра с одним потенциальным нарушителем и одним релевантным типом, $\hat r_i$ можно считать константой $r$, а $\Delta_i$ записать явно; тогда $(★)$ сворачивается в классическую эвристическую форму
$$
r\,\delta \;\ge\; \Delta,
$$
которая удобно визуализирует пространство параметров санкций и показывает, что институт устойчив не в одной точке, а во всей области $(r,\delta)$, где выполняется $(OB$. 

При этом важно различать *объективную* и *субъективную* составляющие вероятности применения санкций. Объективная структура мира задаётся распределением состояний $\psi$; часть состояний $\theta$ может описывать режимы, в которых санкционный механизм действует, другая часть — режимы, где санкции отсутствуют или ослаблены.  В этом смысле величина $\psi(\theta \in \Theta^{\text{санкц}})$ выражает объективную частоту «санкционных» миров. Субъективная вероятность санкции при девиации $\hat r_i(t_i,a_i,a_i')$ — это уже постериор игрока, возникающий внутри условных ожиданий в $(OB)$; она зависит от априори $\psi$, информационной структуры $\pi$ и решающего правила $\sigma$. 
...
Чтобы $\delta$-параметр перестал выглядеть произвольным модификатором полезности и обрёл онтологическое содержание, нужно вписать
...
![Объективная $r$ и субъективная оценка агента $\tilde r(s_i)$ вместе показывают «запас прочности» института](assets/protoinstitute.png){width=70%}
