...
Обсуждая онтологию института, Гуала и Хиндрикс часто говорят о *конвенции*. По их мнению, институт можно описать, как конвенцию с правилами в качестве устройств корреляции: при получении сигнала отклоняться невыгодно, а сигнал — это регулятивное правило вида «если X, делай Y».
...
\text{Не пасти} & 1,2 & 1,1 & 1,1.5 \\
\hline
\text{Пасти, если первый} & 0.5,1 & 1.5,1 & \textcolor{red}{1.5,1.5} \\
\hline
\end{array}
$$
...
Чтобы $\delta$-параметр перестал выглядеть как произвольный модификатор платежа и обрёл онтологическое содержание, нужно определить, «где он находится» на уровне модели. 

Рассмотрим игру с КР. Для каждого игрока $i$ заданы множество действий $A_i$ и множество сигналов — ролевых рекомендаций $S_i$. Базовые выигрыши задаёт функция $u_i: A \to \mathbb R$. Медиатор выбирает профиль сигналов $s = (s_1,\dots,s_n) \in S$ по распределению $\mu \in \Delta(S)$ и сообщают каждому игроку $i$ только его компонент $s_i$. Стратегия игрока $i$ — это функция $\sigma_i: S_i \to A_i,$ которая ставит каждому сигналу $s_i$ в соответствие действие $\sigma_i(s_i)$. Распределение $\mu$ образует КР: для любого игрока $i$, сигнала $s_i$ с $\mu(s_i)>0$ и любого возможного отклонения $a_i' \in A_i$ выполняется условие

$$\mathbb E\bigl[u_i(\sigma_i(s_i),\sigma_{-i}(s_{-i})) \mid s_i\bigr] \ge \mathbb E\bigl[u_i(a_i',\sigma_{-i}(s_{-i})) \mid s_i\bigr],$$

где $\sigma_{-i}(s_{-i}) = (\sigma_j(s_j))_{j \ne i}$.

Так мы получаем КР. Чтобы стабилизировать КР, добавим санкцию. Институт, помимо распределения $\mu$, задаёт $\delta > 0$ — размер санкции. Каждому сигналу $s_i$ сопоставлена стратегия $a_i^{\text{rec}}(s_i) \in A_i$ — что рекомендуется делать при данном сигнале. 

Определим индикатор отклонения от рекомендации:
$$
D_i(s_i,a_i) =
\begin{cases}
1, & \text{если } a_i \neq a_i^{\text{rec}}(s_i),\\
0, & \text{если } a_i = a_i^{\text{rec}}(s_i).
\end{cases}
$$

Ожидаемый выигрыш игрока $i$ при сигналах $s$ и действиях $a$ задаётся как

$$u_i^{\text{inst}}(s,a) = u_i(a) - \delta \cdot D_i(s_i,a_i).$$

Если игрок следует рекомендации коррелирующего устройства, то $D_i=0$ и санкция отсутствует. А если он отклоняется, его выигрыш уменьшается на величину $\delta$.

Тогда условие КР примет вид: 

$$\mathbb E\Bigl[
u_i^{\text{inst}}\bigl(s,\sigma_i(s_i),\sigma_{-i}(s_{-i})\bigr)
\;\bigm|\; s_i
\Bigr]
\;\ge\;
\mathbb E\Bigl[
u_i^{\text{inst}}\bigl(s,a_i',\sigma_{-i}(s_{-i})\bigr)
\;\bigm|\; s_i
\Bigr].$$

Подставляя определение $u_i^{\text{inst}}$, мы получаем

$$\mathbb E\bigl[
u_i(\sigma_i(s_i),\sigma_{-i}(s_{-i}))
\mid s_i
\bigr]
\;\ge\;
\mathbb E\bigl[
u_i(a_i',\sigma_{-i}(s_{-i})) - \delta \cdot D_i(s_i,a_i')
\mid s_i
\bigr].$$

Если $a_i' = a_i^{\text{rec}}(s_i)$, то $D_i=0$, и условие сводится к обычному условию КР. Если $a_i' \neq a_i^{\text{rec}}(s_i)$, появляется $-\delta$ — ожидаемая санкция за оппортунистическое отклонение.

Для каждого сигнала $s_i$ и девиации $a_i'$ можно определить премию за отклонение:

$$
\Delta u_i(s_i;a_i',a_i^{\text{rec}}(s_i))
=
\mathbb E\bigl[u_i(a_i',\sigma_{-i}(s_{-i})) - u_i(a_i^{\text{rec}}(s_i),\sigma_{-i}(s_{-i})) \mid s_i\bigr].
$$

Тогда «институциональное» условие невыгодности отклонения принимает форму:

$$\delta \ge \Delta u_i(s_i;a_i',a_i^{\text{rec}}(s_i)).$$

Эта формула показывает, почему нельзя «превратить любую игру в институт», просто добавив штраф $\delta$: 

- санкция должна быть привязана именно к отклонениям от ролевых рекомендаций, а не вычитаться из всех платежей; 
- санкция $\delta$ должна быть достаточно сильной, чтобы в *конкретной игре* и при *конкретной конвенции* перекрыть премию от оппортунизма. 

В разных играх $\Delta u_i$ будет разной функцией от структурных параметров (например, от $V$ и $C$ в ЯГБ).

Иначе говоря, институт как механизм «надстраивается» над устройством корреляции и делает его более «заметным» (salient) для игроков. Медиатор даёт личные сигналы $s_i$, а институт добавляет публичный и наблюдаемый канал санкций ($D_i$ обнаружимо, штраф $-\delta$), повышая «заметность». Игроки фокусируются на $a_i^{\text{rec}}(s_i)$ как на фокальной точке даже без полного знания чужих сигналов, как этого требует Вандершрааф и, как мы полагаем, Гуала с Хиндриксом. Фокальность усиливает координацию, делая отклонения очень дорогими и видимыми.

В первом приближении можно сказать, что институт — это «слой» санкций над устройством корреляции, делающий следование его рекомендации более выгодным, чем отклонение.

\newpage

## 3.2 Вероятность санкции и область параметров $(r, \delta)$

До этого момента $\delta$ описывалась как санкция, применяемая при отклонении *всегда*. В реальных институтах санкции работают вероятностно: не каждое нарушение фиксируется и наказывается. Поэтому второй параметр нашей модели — надёжность санкций $r \in [0,1]$, понимаемая как вероятность того, что нарушение будет обнаружено и повлечёт наказание. Его можно представить как отношение числа санкционированных нарушений к общему числу нарушений в большом семействе эпизодов. 

Формально это означает, что ожидаемый выигрыш при отклонении вероятностно взвешивает величину санкции: 
$$
EU(g_i) = u(g_i,f_{-i}) - r\delta
$$
Онтологически $r$ отражает структуру мониторинга и принуждения: плотность наблюдения, ресурсы контроля, эффективность процедур и другое. Например, насколько много камер видеонаблюдения на дорожной полосе: чем больше, тем выше вероятность санкции за превышение скорости.

На примере игры ЯГБ с выпасом скота из прошлой главы и стандартными значениями ценности ресурса $V = 50$ и «стоимостью» драки $C = 100$ получим:

$$EU(L,B) = \frac{3V}{4} - r\delta.$$

Условие устойчивости условной стратегии $B$ против блефа становится:

$$\frac{3V}{4} - r\delta \leq \frac{V}{2} \quad\Longleftrightarrow\quad r\delta \geq \frac{V}{4}.$$

Иначе говоря, необходимым и достаточным условием «институционального» КР в этой игре с неполной информацией будет:

$$r\delta \;\geq\; \Delta u, \quad\text{где}\quad \Delta u = \pi(L,B) - \pi(B,B) = \frac{V}{4}.$$

Это важнее, чем величина $\delta$ сама по себе, поскольку мягкая, но почти неизбежная санкция может быть столь же эффективной, как редко применяемая суровая санкция. Если $r\delta < \Delta u$, отклонение остаётся выгодным, и профиль условных стратегий не может быть реализовывать КР.

Это значит, что реально действующая санкция должна быть больше минимально допустимой:

$$\delta_{eff} = r\delta \geq \delta_{min} \quad {\text{при}} \quad \delta_{min} = \frac{V}{4}$$

Соответственно, минимальная частота реализации санкции должна быть:

$$r_{min} = \frac{\delta_{min}}{\delta}$$

Это значение можно назвать *порогом институциональности* поведенческого паттерна: ниже этого значения применение санкций станет непредсказуемым и не приведёт к равновесию.

В этой форме величина $r\delta$ перестаёт быть произвольным «модификатором полезности» и приобретает онтологическое содержание: это параметр институционального механизма, который определяет, насколько рискованно нарушать правило.

В главе 2 мы показали, что в байесовской игре ЯГБ с блефом в развёрнутой форме существует положительная премия $\Delta u$ от отклонения от буржуа‑правила. Ниже мы рассмотрим редуцированную 2×2‑версию игры Ястребы-Голуби, трактуя $\Delta u$ как уже выведенный структурный параметр стимула к нарушению. Это позволяет абстрагироваться от деталей типового пространства и сосредоточиться на институциональном пороге $r\delta \ge \Delta u$.
...
Теперь предположим, что нарушение правила влечёт санкцию $\delta > 0$ с вероятностью $r \in [0,1]$. Тогда ожидаемый выигрыш «захватчика» при нарушении становится:
$$
\mathbb E[u(H)]_{\text{inst}} = 1 - 2p - r\delta.
$$
Институт «собственности» реализован, если следовать правилу (играть $D$ в роли «захватчика») не менее выгодно, чем его нарушать:
$$
\mathbb E[u(D)] \ge \mathbb E[u(H)]_{\text{inst}}.
$$
...
В онтологическом плане $r\delta$ здесь — институциональный слой (право, санкции общины, репутационные потери и т.п.), который компенсирует «сырую» премию $\Delta u(p)$ и делает отклонение от правила невыгодным. При $r\delta < \Delta u(p)$ правило остаётся лишь «бумажной нормой» или «спящим» институтом, тогда как при $r\delta \ge \Delta u(p)$ оно реализуется как *строгое КР*.
...
Чтобы это проиллюстрировать, рассмотрим другой случай — игру «Охота на оленя», где каждый выбирает либо охотиться на оленя $S$ и получить больший выигрыш, либо единолично охотиться на зайца $H$ и получить меньший выигрыш. Пусть матрица игры имеет следующий вид:

<div class="table">
<!--пофиксить матрицу — todo-->
$$
\begin{array}{|l|c|c|}
\hline
& S & H \\
...
\end{array}
$$
<figcaption>Охота на оленя</figcaption>
</div>

Смешанные профили $(S,H)$ и $(H,S)$ описывают ситуации несогласованности: игрок, выбирающий $S$, оказывается с результатом $0$, а его оппонент — с результатом $3$. 

Предположим, что в данной ситуации существует институт, задающий правило «если игрок участвует в охоте, он должен выбирать стратегию $S$ (охотиться на оленя)». Это правило реализуется через устройство корреляции, которое в каждом раунде генерирует профиль сигналов $s = (s_1,s_2)$ согласно некоторому распределению $\mu$ и выдает игроку $i$ индивидуальную рекомендацию $\sigma_i(s_i)$.

Однако проблема в том, что у игроков неполная информация — они не знают сигналы других игроков. Иначе говоря, это *неравновесное состояние* социальной системы, в которой, тем не менее, есть «правила» и предписания действий — ситуация, о которой писал Вандершрааф в критике Гуалы [@Vanderschraaf_2017].

Институциональное правило «охоться на оленя» реализовано тогда и только тогда, когда ни в одном из релевантных информационных состояний отклонение не является выгодным, то есть когда

$$r\delta \ge \Delta u.$$

Устройство корреляции рекомендует сигнал $s = S$ с вероятностью $p$. Тогда при следовании сигналу (играть $S$):
$$\mathbb E[u(S)] = 4p + 0\cdot(1-p) = 4p;$$

А при отклонении (играть $H$):
$$\mathbb E[u(H)] = 3p + 2(1-p) = 2 + p.$$

Премия за отклонение будет:

$$\Delta u(p) = \mathbb E[u(H)] - \mathbb E[u(S)] = (2+p) - 4p = 2 - 3p.$$

Если представить ситуацию «идеального института», где все ожидают, что оппонент играет $S$ с $p = 1$, то:

$$\Delta u(1) = 2 - 3\cdot 1 = -1.$$

Это означает, что при $p=1$ отклоняться невыгодно вообще — даже без санкции. Чтобы появилась премия за отклонение, нужно $p< \frac23$. Например, при $p=\frac12$:

$$\Delta u(0.5) = 2 - 3\cdot \frac12 = 2 - 1.5 = 0.5.$$
...
![Параметры $r$ и $\delta$ компенсируют отклонение при разных вероятностях оппонента играть $S$. Пунктирная линия означает порог $r\delta = \Delta u$](assets/stag_hunt.png)

Это позволяет уже на уровне одной статической игры различить три режима:

1. *Доинституциональный*: $r\delta < \Delta u$. Отклонение от рекомендации медиатора выгодно, условная стратегия уязвима, коллапс института.
2. *Протоинституциональный*: $r\delta = \Delta u$. Отклонение и следование роли дают одинаковый ожидаемый выигрыш. Формально санкция есть, но она лишь нейтрализует премию от отклонения и не создаёт устойчивого перекоса в пользу роли. Пограничное состяние, нестрогое КР.
3. *Институциональный*: $r\delta > \Delta u$. Отклонение строго невыгодно, строгое КР.

Важно, что в играх одного типа (например, «Охота на оленя») возможны как конвенции, так и институты. В обоих случаях устойчивый социальный порядок можно описать посредством строгого КР Ауманна. Различается не целевое состояние, а *механизм* корреляции: в случае конвенции корреляция обеспечивается общим знанием структуры игры и взаимных ожиданий (в смысле Вандершраафа), тогда как в случае института — внешним механизмом принуждения, задающим ожидаемый штраф $r\delta$ за отклонение. Формальном это отражается в том, что и конвенция, и институт реализуют одно и то же множество аумановских КР, но в институциональном случае условие отсутствия стимулов к отклонению принимает вид
$$
...
Иначе говоря, для каждой игры «порог институциональности» будет разным и будет зависеть от структуры выигрышей, а конкретные значения $r\delta$ зависят от устройства корреляции — какие сигналы и как оно посылает участникам.
...
<!--- вставить определение из inst-def.md — todo-->
**Определение 1 (исходная игра с коррелирующим устройством).**
Исходной игрой с коррелирующим устройством называется кортеж
$$
G = \bigl(N,(A_i)_{i\in N},(S_i)_{i\in N},(u_i)_{i\in N},\mu,(a_i^{\text{rec}})_{i\in N}\bigr),
$$
где

- $N$ — конечное множество игроков;  
- $A_i$ — множество действий игрока $i$, $A = \prod_{i\in N} A_i$;  
- $S_i$ — множество сигналов для игрока $i$, $S = \prod_{i\in N} S_i$;  
- $u_i : A \to \mathbb R$ — функция выигрыша игрока $i$;  
- $\mu \in \Delta(S)$ — распределение сигналов коррелирующего устройства на $S$;  
- $a_i^{\text{rec}} : S_i \to A_i$ — рекомендованное действие игрока $i$ при сигнале $s_i$.

Коррелирующее устройство $(S,\mu)$ и функции $a_i^{\text{rec}}$ задают правило координации, но сами гарантируют его исполнения.

Определим индикатор отклонения от ролевой рекомендации:
$$
D_i(s_i,a_i)=
\begin{cases}
1, & a_i \neq a_i^{\text{rec}}(s_i),\\
0, & a_i = a_i^{\text{rec}}(s_i),
\end{cases}
$$
для всех $s_i \in S_i$ и $a_i \in A_i$.

Институциональные выигрыши в расширенной игре задаются как
$$
u_i^{\text{inst}}(s,a)
=
u_i(a) - r\delta \cdot D_i(s_i,a_i),
$$
где $r \in [0,1]$ — надёжность санкций, $\delta>0$ — их суровость.

**Определение 2.**
Институтом на основе исходной игры $G$ называется оператор
$$
\mathcal I_{r,\delta} : G \longmapsto G^{\text{inst}},
$$
который каждой игре с коррелирующим устройством $G$ ставит в соответствие институциональную игру
$$
G^{\text{inst}}
=
\bigl(N,(A_i)_{i\in N},(S_i)_{i\in N},(u_i^{\text{inst}})_{i\in N},\mu,(a_i^{\text{rec}})_{i\in N},r,\delta\bigr),
$$
где $u_i^{\text{inst}}$ определены выше.

Онтологически институт — это механизм $\mathcal I_{r,\delta}$, постоянно реализующий взаимодействие в форме институциональной игры $G^{\text{inst}}$, а кортеж $(G,r,\delta)$ — конкретная реализация этого оператора на заданной базовой игре.

Пусть $\sigma_i : S_i \to A_i$ — (чистая) стратегия игрока $i$, и $\sigma_{-i}(s_{-i}) = (\sigma_j(s_j))_{j\ne i}$.

**Определение 3.**
Пара $(\mu,( \sigma_i )_{i\in N})$ называется институциональным коррелированным равновесием институциональной игры $G^{\text{inst}} = \mathcal I_{r,\delta}(G)$, если для каждого игрока $i \in N$, каждого сигнала $s_i \in S_i$ с $\mu(s_i) > 0$ и любого альтернативного действия $a_i' \in A_i$ выполняется
$$
\mathbb E\Bigl[
u_i^{\text{inst}}\bigl(s,\sigma_i(s_i),\sigma_{-i}(s_{-i})\bigr)
\;\bigm|\; s_i
...
Это стандартное условие КР Аумана, применённое к игре $G^{\text{inst}}$ с ожидаемыми выигрышами $u_i^{\text{inst}}$.

Подставляя определение $u_i^{\text{inst}}$, получаем форму:
$$
\mathbb E\bigl[
u_i(\sigma_i(s_i),\sigma_{-i}(s_{-i}))
\mid s_i
...
\;\ge\;
\mathbb E\bigl[
u_i(a_i',\sigma_{-i}(s_{-i}))
\mid s_i
\bigr]
- r\delta \cdot D_i(s_i,a_i').
$$

Для каждого игрока $i$, сигнала $s_i$ с $\mu(s_i)>0$ и девиации $a_i' \in A_i$ определим премию за отклонение (относительно профиля следования предписаниям $\sigma_i = a_i^{\text{rec}}$):
$$
\Delta u_i(s_i;a_i',a_i^{\text{rec}}(s_i))
=
\mathbb E\bigl[
u_i(a_i',\sigma_{-i}(s_{-i})) - u_i(a_i^{\text{rec}}(s_i),\sigma_{-i}(s_{-i}))
\mid s_i
\bigr].
$$

Тогда профиль $\sigma_i(s_i)=a_i^{\text{rec}}(s_i)$ для всех $i,s_i$ является *институциональным коррелированным равновесием* тогда и только тогда, когда для всех $i,s_i,a_i'$:
$$
r\delta \;\ge\; \Delta u_i(s_i;a_i',a_i^{\text{rec}}(s_i)).
$$

Для краткости обозначим для каждого игрока
$$
\Delta u_i^{\max}
=
\max_{s_i:\,\mu(s_i)>0}\,\max_{a_i'\in A_i}
\Delta u_i(s_i;a_i',a_i^{\text{rec}}(s_i)),
$$
и далее
$$
\Delta u^{\max}
=
\max_{i\in N} \Delta u_i^{\max}.
$$

Тогда условие $\sigma_i = a_i^{\text{rec}}$ быть институциональным КР эквивалентно одному неравенству
$$
r\delta \ge \Delta u^{\max}.
$$
**Определение 4**  
Пусть $G$ и $(S,\mu,(a_i^{\text{rec}})_i)$ фиксированы. Для оператора $\mathcal I_{r,\delta}$:

- $\mathcal I_{r,\delta}$ называется *работающим институтом* относительно предписаний $a_i^{\text{rec}}$, если $r\delta \ge \Delta u^{\max}$ и профиль $\sigma_i = a_i^{\text{rec}}$ образует институциональное коррелированное равновесие институциональной игры $G^{\text{inst}} = \mathcal I_{r,\delta}(G)$;  
- $\mathcal I_{r,\delta}$ называется *критическим протоинститутом*, если $r\delta = \Delta u^{\max}$: следование норме ровно безразлично относительно наилучшей девиации (нулевой запас устойчивости);  
- $\mathcal I_{r,\delta}$ называется *спящим (или плохим) институтом*, если $r\delta < \Delta u^{\max}$: структура сигналов, предписаний и санкций задана, но соответствующая конвенция не является институциональным коррелированным равновесием, и отклонение остаётся выгодным.

В нашей рамке институт — это оператор $\mathcal I_{r,\delta}$ на множестве игр с коррелирующим устройством, а институциональные КР — это стандартные КР Ауманна в игре $G^{\text{inst}}$, порождённой этим оператором.
...
## 3.4 Возможные направления развития модели

Важно, что параметры $r$ и $\delta$ — *эндогенные*: они зависят от параметров игры. В рассмотренных нами случаях — от вероятности следовать сигналу. Однако их можно смоделировать как зависимые от числа игр $N$ (и, следовательно, $M$), «инвестиций» части популяции, заинтересованной в стабильности условной стратегии, в санкции и мониторинг их исполнения и поддержание достаточного уровня $r\delta$ или другими способами. 
...
С онтологической точки зрения, именно $r$, а не только $\delta$ отражает глубинную природу института. Ключевая задача инфраструктуры фиксации и обработки нарушений — *масштабирование санкции*. Это особенно хорошо видно, если представить институт как сеть эпизодов (или игр), как в части 3.3. 
...
Эффективность подестерии объясняется *унификацией санкций*: в отличие от кровной мести, где наказание варьируется по кланам (субъективные возмездия, ритуалы чести), генуэзские статуты стандартизировали штрафы и изгнания, применяемые подестой, что снижало цикл эскалации. Кровная месть фрагментирует порядок, тогда как подеста *централизует принуждение*, превращая клановую силу в сдерживающий фактор, а не инструмент захвата. Другими словами, унификация санкций и единый мезанизм их «доставки» обеспечивали институту подесты стабильность.
...
Равновесия Ауманна в этой перспективе — не определение института, а язык для описания устойчивых паттернов, возникающих благодаря санкционному механизму. Можно сказать, что условная стратегия реализует КР в игре, модифицированной санкциями. Но равновесие оказывается *следствием* того, что ожидаемый штраф перекрывает премию за отклонение, а не первичным объяснением принудительной силы. Равновесия — это компактные *описания* устойчивых паттернов поведения, а не отдельные сущности, к которым редуцируется институт.
...
У нашей онтологической рамки также есть преемственность с линией Остром/Норта. С одной стороны она номинальная, а с другой — сущностная. Номинальная заключается в том, что мы, вслед за Гуалой, используем понятие «правило», но в нашей рамке оно лишь даёт язык описания, а не онтологическую сущность. В нашей рамке правило — это функция из сигнала (или возможного мира) в стратегию $\Omega → S$. Дополнительного значения «рецепта действия» и тем более объяснения содержательной и нормативной структуры правил наша рамка не предусматривает. Однако с содержательной точки зрения наша рамка близка идеям Остром о связи правил, мониторинга и градуированных санкций для поддержания институционального порядка [@ostrom1990; @ostrom2005]. Если для Остром эта связка важна для эмпирических исследований, то для нас мониторинг и санкции составляют онтологическое ядро института как обнаружимого социального порядка. Конвенция, например, не требует подобной инфраструктуры.

Наконец, наша рамка согласуется с идеями Гинтиса и Биккиери об интернализованных нормах как слабом принуждении. Наша модель не ограничивает возможные источники санкции $\delta$, что означает, что они могут быть интернализованными — виной, стыдом и нормами как ожиданиями второго порядка («я ожидаю, что другие ожидают от меня определённое поведение»). Также идея Гинтиса о матрице верований (belief-matrix) которая изменяет матрицу изначальной игры, получает новое прочтение — источником модификации платежа могут быть не только (и не столько) верования, сколько социоматериальная инфраструктура принуждения, где нормы и верования — лишь часть. Можно сказать, что нормы и верования — это социальные технологии повышения $r$, которые обеспечивают осведомлённость о возможных санкциях. И это работает на разных уровнях: неподобающее поведение в библиотеке так же повлечёт санкцию, как и нарушение закона, просто её источник, сила и вероятность будут другими. 
...
Важный для Гуалы тезис — что поскольку институт онтологически не зависит от ментальных состояний, его можно было бы назвать естественным видом — получает в нашей рамке уточнение. Если в онтологии Гуалы видовое свойство института — функция координации поведения через общее знание правил, то в нашей — это инфраструктура мониторинга, делающая следование правилу выгоднее отклонения. Более того, поскольку наша рамка даёт параметры, она поддерживает индуктивную генерализацию: зная параметры конкретного института, мы можем предсказывать, при каких значениях этих параметров он будет коллапсировать или набирать силу. Однако мы бы не стали утверждать, что это делает институт естественным видом в сильном смысле Милля-Бойда. Несмотря на то, что параметрическую структуру $\Delta u , r, \delta$, которая предполагает наличие информационного канала о фактах применения санкции, можно считать гомеостатическим кластером свойств, она всё равно зависит от верований, ожиданий и способности агентов интерпретировать информацию о $\delta$. Другими словами, это по-прежнему интерактивный вид Хакинга, но с частью, напоминающей естественный вид.
...
Ниже мы покажем, что наша модель, разработанная в прошлых главах, помимо своей унифицирующей роли для теорий конвенций в философии и теорий институтов в социальной онтологии и экономике может выполнять роль такого объединяющего принципа для социологии. Это то, чем теория Гуалы предложить не может, поскольку она не описывает принудительный социальный порядок. Мы также обозначим нашу позицию как *умеренный эпистемический структурный реализм* о социальном порядке, который (1) уточняет линию Росса и других натуралистически настроенных философов социальной науки, (2) даёт общий каркас для классических теорий социологии, и (3) показывает, чем может быть структурное ядро социологии.

ОСТАНОВИЛСЯ ЗДЕСЬ

Унификация и натурализация онтологии — разные задачи. Первая подводит объяснения разных классов фактов под одну схему, углубляя онтологическую картину домена, а вторая предполагает построение онтологии с помощью терминов самой науки. 

Гуала — один из немногих, кто пытается сделать обе вещи. 

---

Проекты натуралистической социальной онтологии [@kaidesoja2013b; @guala2016a; @kincaid2024], и теория правил-в-равновесии, о которой мы уже много сказали — один из самых заметных представителей. Она строится в терминах экономики — координационных равновесий и правил — и почти не предполагает онтологических обязательств, служа прагматической и дефеляционной онтологией, а не полнокровной метафизикой социального. Однако, как мы показали ранее, даже она не достигает онтологической унификации, ограничиваясь деривационной (или объяснительной).

Более глубокая проблема заключается в том, что, как считают некоторые философы, натуралистическую социальную онтологию в духе Росса представить сложно. Для этого должен быть доступен вывод от *объяснительно успешных* теорий к общей онтологии, а эти теории должны обладать предсказательной силой [@psillos1999]. Это неявно предполагает, что между теориями в дисциплине есть *общее ядро* или объединяющий принцип вроде теории эволюции Дарвина в биологии или теории рационального выбора в экономике. 

Наличие объединяющего принципа позволило бы социальным наукам достичь более высокой объяснительной и прогностической силы. Хотя некоторые ученые утверждают, что многопарадигмальный характер социологии — это приятная особенность, а не недостаток, поскольку социальная реальность сложна и в контекстуальна [@little2016], это всё же препятствует кумулятивному росту знаний [@genov2019]. По меркам Томаса Куна [@kuhn2009], социология — это не зрелая наука, а преднаука [@porpora2015]. И, как предположил Кун, в таком состоянии науки уместно продолжать искать о наукибщедисциплинарный консенсус.

Некоторые философы считают, что метод IBE принципиально недоступен для социальных наук и социологии, в частности, поскольку в ней «нет аналогов гравитационных волн или бозона Хиггса» [@hawley2018]. Нет фундаментальных единиц анализа, исходя из которых можно выводить онтологию по лекалам научного реализма [@psillos1999]. 



### 4.3.1 Возможность вывода к наилучшему объяснению в социологии

В социальной науке до сих пор есть расхождение между парадигмами, — от рационального выбора до герменевтики — и этого достаточно, чтобы сказать, что натуралистическая социальная онтология неправдоподобна как проект [@hawley2018]. Этот аргумент парируют, приводя в качестве примера социальную эпидемиологию и гипотезу о неравенстве доходов (что неравенство ухудшает здоровье популяции в целом). Эта гипотеза, как пишет автор, обладает предсказательным успехом и, следовательно, может способствовать развитию онтологии [@saunders2020]. Однако несмотря на оптимизм, этот аргумент бьёт мимо цели: хотя предсказательного успеха одной теории формально достаточно для предположения о существовании механизмов, которые она описывает, это далеко от натуралистической социальной онтологии в понимании Росса, к которой мы стремимся. Практический смысл подобной онтологии — *в онтологическом и объяснительном объединении домена*, а не положения одной теории как онтологически правдоподобной. А это означает, что для IBE нужна унифицированная доменная теория.

Другой, но связанный вопрос — как онтология связана с методологией. Существует спор о том, что из них является первичным. Опираясь на работы @epstein2015, @lohse2017 и @searle2008, @lauer2019 критикует идею о том, что социальная онтология предшествует методологии социальных наук. Например, Сёрл отмечает, что социальная онтология предшествует методологии и теории: «Она предшествует в том смысле, что, если у вас нет четкого представления о природе изучаемых явлений, вы вряд ли сможете разработать правильную методологию и правильный теоретический аппарат для проведения исследования» [-@searle2008, 443]. Эпштейн выражает аналогичную озабоченность: «Онтология имеет последствия, и онтологические ошибки приводят к научным ошибкам. Убеждения относительно природы сущностей в науке — как они состоят, от каких сущностей они онтологически зависят — вплетены в модели науки» [-@epstein2015, 127]. Онтология влияет на методологию в том смысле, что определяет релевантные аспекты рассматриваемого явления, которые влияют на подход к нему. Однако определить эти аспекты априори сложно. Социальная онтология не предшествует социальной методологии, поскольку релевантные аспекты явлений подлежат эмпирической проверке, и это относится и к социальным наукам.

Литтл отвергает априорную позицию и утверждает, что социальная онтология не может быть оправдана на неэмпирической основе [@little2020] . В противовес ряду философов [@lauer2019; @epstein2015; @searle2010], он утверждает, что социальная онтология — это абстрактная граница социально-научного теоретизирования и что она итеративно связана с эмпирическим исследованием. Эта позиция предполагает аналогичный индуктивный процесс, как в естественных науках, и в принципе поддерживает вывод от теории к онтологии. 

Однако Литтл явно отвергает «единство научной онтологии», подразумеваемое в рассуждениях Хоули, которая предполагает вывод из «теоретического ядра» науки. Первое подразумевает плюрализм и исследование социальной онтологии в каждом конкретном случае, подобно контекстуализму Кинкейда [@kincaid2021], а второе подразумевает монизм в отношении основного набора сущностей и отношений: существуют «основные» части, применимые повсеместно. Литтл отрицает возможность существования такой полной социальной онтологии, которая могла бы служить основой для социально-научного рассуждения. Как он утверждает, в социальной реальности нет «атомов» или фундаментальных элементов. Литтл также отрицает идею о существовании окончательного списка сущностей, из которых состоит социальный мир и в рамках которого следует переформулировать все привычные социально-научные концепции. Другими словами, реализм Литтла гораздо слабее научного реализма, подразумеваемого Хоули, и именно поэтому он не видит проблемы в использовании реализма для социальной онтологии: это всего лишь два разных понятия с одним и тем же названием.

В противовес Литтлу, мы полагаем, что единство научной онтологии в социологии возможно — как минимум в слабом смысле, и для этого не нужны ни «список сущностей», ни фундаментальные «атомы». Другими словами, Литтл предполагает онтологию *сущностей* и упускает из виду возможность онтологии *структур*, которые могут отражать одни и те же паттерны «в мире» вне зависимости от контекста исследования.

Структурный реализм утверждает реальность математических структур, которые схватывают «реальные паттерны» и отношения в мире [@ladyman2007]. Изначальная формулировка этой позиции в работах Джона Уоррала не давала понять, является ли эта позиция эпистемологической или метафизической: тезис Уоррала заключался в том, что при смене научных парадигм теории могут кардинально менять содержание, при этом сохраняя математическую структуру [@worrall1989].

В частности, Уорралл интерпретирует переход от оптики Френеля к уравнениям Максвелла как демонстрацию структурного реализма: онтология радикально изменилась — уравнения Френеля описывали свет как поперечную упругую волну в механическом эфире, тогда как Максвелл описал электромагнитное поле, в котором световая волна —  частный случай [@worrall1989]. Математическая структура — соотношения между интенсивностями, коэффициентами отражения и преломления — осталась в значительной мере сохранённой.

Позднее, Джеймс Ледимен сформулировал различение *онтического* (ОСР) и *эпистемического* (ЭСР) структурного реализма, где первый утверждает, что существуют только структуры (охватываемые теориями паттерны отражают реально существующие отношения в мире), а второй — что математическая структура успешных теорий эмпирически адекватна и мы можем знать структуру мира [@ladyman1998]. ОСР предполагает серьёзные онтологические обязательства — что существуют только структуры, и есть много различающихся формулировок этой позиции [@frigg2011]. Их мы оставим в стороне, поскольку для них метафизика важнее, чем прагматичность и влияние на кумулятивный рост научного знания, которые важны для нас.
...
<!--Чтобы обосновать идею о том, что с точки зрения объяснения индивиды имеют меньшее значение, чем их роли и позиции, он использует теоретико-множественное разграничение структурной эквивалентности и структурного изоморфизма, причем первое означает тождественность как индивидов, так и их отношений, а второе — только тождественность отношений между их ролями.-->
<!---->
<!--Кинкейд утверждает, что социальные структурные отношения не являются причинными, а концептуальными. Роли и институты определяются через друг друга, например, профессора — это ученые, преподающие в университетах, а университеты — это места, где студенты учатся у профессоров. Таким образом, структурные отношения являются концептуальными, а не причинными.-->

Росс считает…

---
Предлагаю аккуратный «мост» от Кинкейда/Росса к твоей рамке, вшитый в уже имеющийся текст и формулирующий вклад в терминах структурного реализма.

***

Кинкейд утверждает, что структурный реализм уже реализован в социальных науках [@kincaid2008]. В социологии это понятие «социальной структуры», предполагающее отношения между ролями и позициями, а не между индивидами. Его стратегия — показать, что социальные объяснения часто опираются на такие структурные отношения (между классами, организациями, позициями в сети), причём эти отношения могут быть сохранены при смене теорий агентов и мотивации. В этом смысле социальная наука уже реализует эпистемический структурный реализм: мы знаем и используем структуру, даже если спорим о том, «что именно за сущности» её реализуют. [jstor](https://www.jstor.org/stable/10.1086/594517)

Росс развивает натуралистический проект более радикально. Он утверждает, что для наук о человеке и обществе возможна единая научная онтология, совместимая с физикализмом, и защищает реализм относительно «экономических» и «социальных» паттернов как относительно реальных, многомасштабных структур, выявляемых кросс‑дисциплинарными моделями [@ladyman2007]. Для него ключевым является не список социальных сущностей, а устойчивые, эмпирически подтверждаемые паттерны, которые играют объяснительную роль в разных науках — от экономики до эволюционной биологии и социологии. [academia](https://www.academia.edu/19103837/James_Ladyman_and_Don_Ross_Every_Thing_Must_Go_Metaphysics_Naturalized)

Однако и Кинкейд, и Росс в основном работают с уже сложившимися категориями социальных наук: классами, институтами, нормами, ролями, полями. Они показывают, что реализм относительно структур возможен, и что социальные науки фактически используют структурные объяснения, но не предлагают *единого структурного инварианта* социального порядка, который мог бы играть роль того, чем для Уоррала была общая математическая структура в переходе от Френеля к Максвеллу [@worrall1989]. Иначе говоря, остаётся открытым вопрос: существует ли в социологии небанальный «общий формал» социального порядка, относительно которого реализм был бы не просто локальным (вокруг отдельных институтов или сетей), а доменным.  

Наша онтологическая рамка делает скромный вклад в эту линию исследований. Она утверждает, что любой устойчивый социальный порядок — от взаимодействий лицом-к-лицу и до международных институтов — реализует одну и ту же абстрактную структуру: конфигурацию вероятностных санкций и механизмов мониторинга, распределённую по ролям, которая делает следование заданному правилу равновесным и устойчивым. Различаются не сами структурные формы, а значения параметров и конкретные материальные, организационные и символические носители этой конфигурации — от «потери лица» в микровзаимодействиях у Гофмана до формализованных правовых санкций в веберовской бюрократии и макроинститутах. [d.umn](https://www.d.umn.edu/cla/faculty/jhamlin/4111/Readings/GoffmanEmbarrassment.pdf)

В этом смысле выявляемая моделью структура претендует не просто на удобный язык описания, а на *структурный инвариант социального порядка как такового*: онтологически сильный настолько, чтобы утверждать необходимость — там, где есть устойчивый социальный порядок, должна быть реализована некоторая конфигурация выигрышей, санкций и инфраструктуры наблюдения, делающая следование правилам равновесным, — но не настолько сильный, чтобы обязать нас к онтическому структурному реализму или исчерпывающей метафизике социальных сущностей [@ladyman1998][@frigg2011].  

Онтологическая ставка состоит в том, что такая структура не является чисто инструментальной фикцией модели, а выражает *общий формал реально существующей социоматериальной организации порядка*: различаются значения параметров и конкретные носители (право, мораль, символическое насилие, сетевые санкции), но не тип структуры. В этом смысле моя позиция продолжает натуралистические программы Кинкейда и Росса, но уточняет их фокус: я защищаю реализм не относительно «институтов» или других специализированных сущностей, а относительно единой, масштабно‑инвариантной (scale‑free) структуры социального порядка, которая должна быть реализована на всех уровнях — от interaction order Гофмана до макроисторических процессов в духе Тилли — если мы хотим, чтобы успех равновесных и структурных объяснений в социальных науках не был чудом. [opo.iisj](https://opo.iisj.net/index.php/osls/article/download/1855/2312/12875)

Таким образом, «онтологический» компонент рамки состоит в утверждении, что существует реально действующий структурный инвариант — конфигурация выигрышей, санкций и механизмов мониторинга, делающая следование правилам равновесным, — который проявляется в различных областях социальной жизни. Этот инвариант впервые был чётко артикулирован в экономике (через теорию игр и равновесий), но его эмпирическое подтверждение и калибровка происходят в гораздо более широком междисциплинарном поле, включая социологию норм, институтов и микро-взаимодействий. В этом смысле я переношу в социологию не экономическую метафизику, а обобщённое, эмпирически подкреплённое знание о структуре социального порядка, вписываясь в традицию структурного реализма, но смещая её акцент с сущностей на единую, онтологически нагруженную структуру.
---



Наша онтологическая рамка делает скромный вклад в эту линию исследований. Она утверждает, что любой устойчивый социальный порядок — от взаимодействий лицом-к-лицу и до международных институтов — реализует одну и ту же абстрактную структуру: конфигурацию вероятностных санкций и механизмов мониторинга, распределённую по ролям, которая делает следование заданному правилу равновесным и устойчивым. Различаются не сами структурные формы, а значения параметров и конкретные материальные, организационные и символические носители этой конфигурации — от «потери лица» до конституционного права. 

В этом смысле структура, выявляемая моделью, претендует не просто на удобный язык описания, а на социального порядка как такового; онтологически сильный настолько, чтобы утверждать необходимость: там, где есть устойчивый социальный порядок, есть и реализация этой структуры, но не настолько сильный, чтобы обязать нас к онтическому структурному реализму или исчерпывающей метафизике социальных сущностей.

Онтологическая ставка состоит в том, что такая структура не является чисто инструментальной фикцией модели, а выражает общий формал реально существующей социоматериальной организации порядка; различаются только значения параметров и конкретные носители, но не тип структуры. В этом смысле моя позиция продолжает натуралистические программы Кинкейда и Росса, но уточняет их: я защищаю реализм не относительно «институтов» или других специализированных сущностей, а относительно единой, scale-free структуры социального порядка, которая должна быть реализована на всех уровнях — от interaction order Гофмана до макроисторических процессов в духе Тилли — если мы хотим, чтобы успех равновесных объяснений в социальных науках не был чудом.

Таким образом, «онтологический» компонент моей рамки состоит в утверждении, что существует реально действующий структурный инвариант — конфигурация выигрышей, санкций и инфраструктуры наблюдения, делающая следование правилам равновесным, — который проявляется в различных областях социальной жизни. Этот инвариант впервые был чётко сформулирован в экономике, но его эмпирическое подтверждение и калибровка происходят в гораздо более широком междисциплинарном поле, включая социологию норм и институтов. В этом смысле я переношу в социологию не экономическую метафизику, а обобщённое, эмпирически подкреплённое знание о структуре социального порядка.
...
# Заключение
Первую главу мы начали с вопроса о том, что такое социальный институт и каков источник его принудительной силы? Речь шла о противостоянии двух линий — менталистской, выводящей принуждение из коллективных убеждений и правил, и экономико‑игровой, трактующей его как функцию структуры стимулов и равновесий. Мы приняли сторону натуралистической социальной онтологии: онтология института должна выводиться не из анализа понятий, а из тех моделей, которые наилучшим образом объясняют устойчивость и принудительность социального порядка.

Построенная в главах 2–3 параметрическая модель позволяет теперь ответить на исходный вопрос в более технически строгой форме. Принудительная сила института — это не «тёмная нормативность» и не чистый эффект верований, а устойчивое неравенство между выгодой от отклонения и ожидаемым наказанием за отклонение. Институт существует там и тогда, где есть социоматериальная инфраструктура, которая делает следование условной стратегии в среднем лучше, чем её нарушение, в широкой сети сходных ситуаций. В этой формулировке равновесие и правила по‑прежнему играют важную роль, но уже не как онтологические основания, а как языки описания структур, порождённых санкционным механизмом.

С точки зрения методологии науки это позволяет нам применить тот же принцип вывода к наилучшему объяснению, который обсуждался в первой главе. Среди конкурирующих программ — «институт как равновесие», «институт как система правил» и «институт как правило‑в‑равновесии» — наилучшим объяснением устойчивости и принудительности оказывается не одна из них в чистом виде, а структурная модель, объединяющая их сильные стороны. Она заимствует у равновесных подходов идею стратегической устойчивости, у теорий правил — внимание к нормативным описаниям и фокальным точкам, но добавляет недостающий слой: параметры \(r\) и \(\delta\), описывающие плотность фиксации нарушений и суровость наказаний, и их отношение к структурной премии за отклонение \(\Delta u\).

Таким образом, в духе онтического структурного реализма Росса, институты предстают как **естественные виды** не потому, что ответствуют нашим интуитивным категориям, а потому что играют устойчивую роль в наилучших объяснениях наблюдаемой социальной стабильности. Их «видовыми свойствами» в этой рамке оказываются не абстрактные «правила» и не «коллективные интенции», а параметры социоматериальной инфраструктуры: насколько часто нарушения фиксируются, насколько ощутимы их последствия и как эти факты переплетены между собой в сети стратегических взаимодействий. Именно это структурное ядро — конфигурации \((r,\delta,\Delta u)\), обеспечивающие устойчивые коррелированные равновесия, — и задаёт ту часть реальности, которую разные социологические теории затем описывают на своём языке.

Важно подчеркнуть, что такая онтология не редуцирует социальное к теории игр. Напротив, она рассматривает игровые модели как эвристический инструмент для реконструкции минимального механизма принудительности, сопоставимого по объяснительной роли с «естественным отбором» в биологии. Социология остаётся незаменимой в описании мотиваций, значений, форм господства и сопротивления; модель, предложенная здесь, лишь задаёт структурный фон, на котором эти феномены становятся понятными как вариации одного и того же институционального механизма.

В этом смысле наша унифицированная параметрическая онтология замыкает методологическое движение, начатое в первой главе. От критики аналитической социальный онтологии и вопроса о возможных «естественных видах» мы пришли к конкретному кандидату: институт как социоматериальная инфраструктура фиксации нарушений и доставки наказаний, делающая определённый класс условных стратегий устойчивым. Этот кандидат не претендует на окончательную истину, но, в терминах IBE, предлагает лучшее на сегодня объяснение того феномена, с которого мы начали: того, как институты приобретают и удерживают принудительную силу.
