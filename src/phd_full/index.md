...
Ниже мы представим формальное описание института как механизма, изменяющего структуру выплат игры и реализующего строгое БКР с помощью вероятностных санкций. При этом институт и конвенция окажутся альтернативными реализациями социального порядка, а не одним и тем же, как считают Гуала и Хиндрикс.
...
В простом случае, рассмотренном выше, игроки исходят из априорного распределения по двум состояниям мира $\theta \in \{0,1\}: P(\theta=1)=r \quad и \quad P(\theta=0)=1−r$.  $\mathbb{P}(\theta=1)=r$, и сигналы медиатора не несут информации о том, действует ли санкционирующий механизм. Такая картина института статична: игроки не обновляют верования, и параметр $r$ выступает как *объективная* частота реализации мира с активным механизмом принуждения.

Более реалистичная картина — допустить, что сигналы *информативны* относительно состояния мира $\theta$. Это значит, что агенты могут обновлять свои оценки вероятности работы санкционирующего механизма после получения сигнала (точнее, условно на сигнал). Тогда релевантный для игрока множитель ожидаемой санкции станет $\tilde r(s_i)\delta$, где $\tilde r(s_i)=\mathbb{P}(\theta=1\mid s_i)$. 
...
\hline 
 & \textbf{H} & \textbf{D} \\ 
\hline 
\textbf{H} & (0,0) & \cellcolor{lightgray}(2,1)^{\omega_2} \\ 
\hline 
\textbf{D} & \cellcolor{lightgray}(1,2)^{\omega_1} & (1,1) \\
\hline 
\end{array}
$$
<figcaption>Игра с выпасом скота (Ястребы-Голуби) с реализованным КР, обозначенным как серые клетки. Адаптировано из @Vanderschraaf_2017</figcaption>
</div>
...
![Отношение премии и вероятности следования сигналу в «Ястребы-Голуби» для стантартной матрицы описывается уравнением $\Delta u(p) = 1 - 2p$. Чем меньше вероятность, что оппонент последует сигналу, тем выше возможная премия.](assets/hd-line.png)
...
Это позволяет представить ситуацию, где фактически обеспеченный санкционный механизм слабее, чем вера агентов в него. Если положить, что субъективная оценка вероятности работы санкционного механизма систематически выше реальной (например, $\tilde r = 1.5r$), то институт будет устойчив даже при деградировавшей или нестабильной санкционной инфраструктуре.
...
Это означает, что агенты *переоценивают* вероятность санкций за отклонение от правила. Голубая пунктирная линия на графике показывает субъективно воспринимаемый порог стабильности института. В зоне под ней институт в глазах агентов коллапсирует, а над ней — работает, даже несмотря на то, что объективной инфраструктуры для этого уже недостаточно. Область между этими линиями — институт как *ожидаемый порядок*, а не как *самоподдерживающееся равновесие*. Чтобы институт функционировал именно как самоподдерживающееся равновесие в строгом смысле, реальный и субъективный уровни вероятности санкций должны находиться в одной и то же области. То есть, должны одновременно выполняться неравенства:
$$
...
$$
\mathcal{P} = \bigl\{ (r,\delta) \;\bigm|\; r\delta < \Delta u(p) \leq \tilde{r}\delta \bigr\},
$$
...
Формально введём *функцию издержек* $k(r,\delta)$, показывающую, во сколько обходится поддержание уровней надёжности $r$ и суровости $\delta$. Эти издержки несёт Инвестор.

Другая вещь, которая вытекает из эндогенности $r$ — что надёжность санкции $r$ снижает минимальную жёсткость санкции, нужную для поддержания условной стратегии в равновесии. Санкция вряд ли будет непропорционально высокой, поскольку она имеет стоимость поддержания $k(\delta)$, которую «оплачивает» Инвестор. 
...
В следующей главе мы встроим идею критерия устойчивости и теоретико-игровой каркас социальног института в более широкий контекст философии социальной науки и натуралистической социальной онтологии. Мы покажем, как предложенная схема позволяет уточнить и расширить унифицирующую теорию правил-в-равновесии Гуалы и Хиндрикса, как эта схема даёт основания для  структурно-реалистисческой онтологии социальных институтов и как она согласуется с некоторыми классическими социологическими теориями.
