...
Равновесие в условных стратегиях с модификатором платежа $\delta$ стабильно только *локально*, в одной игре, но рушится при наличии множества игр — например, на пространственной сетке. Различение локальной и глобальной ЭСС позволяет ближе подойти к онтологическим основаниям института и отделить его от прототинститута, чего ни одна теория из расмотренных нами в Главе 1 не делает.

Локальная ЭСС означает устойчивость стратегии в одной игре $𝐺_i$. Глобальная устойчивость — в популяции, где много экземпляров игры $G_N$ происходят в разных местах (и/или моментах). Это пространственная множественность, которую можно отразить на пространственной сетке, или временная повторяемость:
...
Пусть $x_i$ — доля популяции, использующая стратегию $i$, $f_i$ — ожидаемый выигрыш стратегии $i$. Уравнение репликатора:
...
При $\delta = \frac{V}{4} = 12.5$ условная стратегия Буржуа перестанет быть ЭСС, и популяция перейдёт в смешанное равновесие:

![Доля стратегий Буржуа и Блефующий при $\delta = \frac{V}{4} = 12.5$](assets/BL_delta-equal.png)

А при $\delta_{min} = 12.64$ Буржуа вновь займёт максимальную долю популяции и станет ЭСС.

![Доля стратегий Буржуа и Блефующий при $\delta > \frac{V}{4} = 12.64$](assets/BL_delta-min.png)

При $\delta = 37.5$ Буржуа также за
...
Рассмотрим случай с множеством игр. Для удобства расположим игры на пространственной сетке, однако мы не будем добавлять сетевые эффекты — ситуацию, когда соседние игры влияют друг на друга, копируя успешные стратегии [@alexander2009]. Подобное допущение включало бы топологию сети как источник асимметрии, а в нашем примере он уже есть — это некий признак вроде очерёдности прибытия на оспариваемую территорию[^asymmetry].

[^asymmetry]: Наша модель позволяет использовать и сетевые эффекты в качестве асимметрии в духе Скирмса [@skyrms2010a], однако подобная модель модель требует отдельного рассмотрения, которое находится за пределами нашего базового аргумента.

На изображении ниже — сетка 20х20, где каждая клетка — одна игровая ситуация. Правила для всех клеток одинаковы, эмпирические матрицы — разные. Цветом обозначена «победившая» в нынешнем раунде стратегия. Стратегии и матрица выигрышей остаются прежними, добавляется лишь параметр $r$ — надёжность санкции в популяции игр. 

<!--- сделать картинку (или анимацию в модели) с сеткой 20*20 — todo-->

Чтобы Буржуа была глобальной ЭСС в популяции, нужна либо высокая санкция $\delta = C = 100$ при минимальном уровне надёжности $r = 0.1251$, либо наоборот — $r = 1$ и $\delta = 12.51$. При $\delta = \frac{V}{4} + 0.01 = 12.51$ и $r \leq 0.99$ профиль условных стратегий Буржуа перестаёт быть глобальной ЭСС, и блеф внедряется в популяцию. Если хотя бы в одной игре из 40 минимальная санкция не применяется к нарушению или если максимальная санкция применяется только в 12.5% игр популяции, Буржуа перестаёт быть ЭСС и равновесие рушится. Показать это динамически также можно с помощью репликаторной динамики. 

<!--- вставить репликаторную динамику для д=12.51 и р=0.99  — todo-->

Можно заметить, что отношение $r$ и $\delta$ можно выразить как функцию: 
$$f(s) = $$
<!--- доделать функцию (мб она и не нужна) — todo-->
...
<!--- вставить матрицу игры гуалы с нулевыми стратегиями блефа, чтобы сымитировать гуалу - todo-->
