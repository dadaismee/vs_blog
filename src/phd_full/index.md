...
\hline 
\textbf{L} & 0 & V & \textcolor{red}{\frac{3V}{4} - \delta} & \frac{V - С}{2} \\ 
\hline 
\end{array}
...
При $\delta > \frac{V}{4} = 50/4 + 0.01  = 12.51$ профиль с условными стратегиями $(B, B)$ будет ЭСС и противостоит внедрению блефующей стратегии. Минимальное нужное значение — именно $12.51$, поскольку при $12.5$ платёж Блефующего ($25$) будет равным платежу Буржуа ($25$), чего недостаточно для ЭСС — одна стратегия должна давать строго больший выигрыш: $f(I, I) > f(J, I)$ [@maynardsmith1982]. Если снизить значение $\delta$, равновесие разрушится и перейдёт к флуктуации — мерцанию между разными исходами. 

ОСТАНОВИЛСЯ ЗДЕСЬ

Стоит подчеркнуть, что $\delta \geq 12.51$ защищает популяцию, состояющую из Буржуа, от *редкого мутанта* со стратегией блефа, но не гарантирует защиту в другой ситуации. Это означает, что подобное значение подходит для *статической проверки* ЭСС. В более реалистичных — динамических — условиях — этого будет недостаточно. Наглядно показать это можно с помощью *репликаторной динамики* — для более высоких и более низких значениях $\delta$ в популяции, почти полностью состоящей из Буржуа. 
...
В нашем эксперименте вычисляется доля популяции стратегий $B$ и $L$ в игре с двумя стратегиями. В нашем случае доля Буржуа в популяции — $x_0 = 0.98$.

При $\delta = 0$ условная стратегия Буржуа перестаёт быть ЭСС и блеф захватывает популяцию:

![Доля стратегий Буржуа и Блефующий при $\delta = 0$](assets/BL_delta-zero.png)

При $\delta = 12.5$ условная стратегия Буржуа перестанет быть ЭСС, и популяция перейдёт в смешанное равновесие, которое не будет ЭСС:
