...
$$
Онтологически $r$ отражает структуру мониторинга и принуждения: плотность наблюдения, ресурсы контроля, эффективность процедур и другое. 
...
$$EU^{r,\delta}(L,B) = \frac{3V}{4} - r\delta.$$

Условие устойчивости условной стратегии против блефа становится:

$$\frac{3V}{4} - r\delta \leq \frac{V}{2} \quad\Longleftrightarrow\quad r\delta \geq \frac{V}{4}.$$

Иначе говоря, необходимым и достаточным условием этого «институционального» КР в этой игре с неполной информацией будет:

$$r\delta \;\geq\; \Delta u, \quad\text{где}\quad \Delta u = \pi(L,B) - \pi(B,B) = \frac{V}{4}.$$

Это важнее, чем сама по себе величина $\delta$: слабая санкция, но при высокой надёжности ($r$ близко к 1) может быть столь же эффективна, как суровая санкция при низкой надёжности. Если $r\delta < \Delta u$, блеф остаётся выгодным, и условная стратегия не может быть устойчивой.
...
Только в этой форме величина \(r\delta\) перестаёт быть произвольным “модификатором полезности” и приобретает онтологическое содержание: это параметр институционального механизма, который определяет, насколько рискованно нарушать правило в данной сети взаимодействий. Как и в правовой экономике, здесь \(r\) можно интерпретировать как частоту \(\Pr(\text{санкция}\mid\text{нарушение})\), обусловленную плотностью мониторинга, ресурсами контроля и эффективностью процедур наказания.

Важно подчеркнуть, что из одного конкретного примера (например, игры ястреб–голубь–буржуа с параметрами \(V\) и \(C\)) нельзя вывести универсальное числовое пороговое значение \(r\delta\). Этот пример служит не для того, чтобы зафиксировать конкретный коэффициент, а для иллюстрации общего принципа: в любой игре ожидаемый штраф \(r\delta\) должен перекрывать премию от отклонения от конвенции. В игре ястреб–голубь–буржуа премия за блеф против буржуа равна \(V/4\), что даёт частный порог \(r\delta \ge V/4\); в других играх премия и, следовательно, порог будут иметь другую форму. [ppl-ai-file-upload.s3.amazonaws](https://ppl-ai-file-upload.s3.amazonaws.com/web/direct-files/attachments/1328076/4bb36c48-3daa-49f5-87b0-07e2f772a4fd/index.md)

ОСТАНОВИЛСЯ ЗДЕСЬ


<!--- институт существует когда r\delta > \Delta и рисуешь пространство (\delta,r) -->

**Определение.** Пара $(\mu,(\sigma_i)_i)$ называется институциональным коррелированным равновесием, если для каждого игрока $i$, каждого сигнала $s_i \in S_i$ с $\mu(s_i)>0$ и любого альтернативного действия $a_i' \in A_i$ выполняется
