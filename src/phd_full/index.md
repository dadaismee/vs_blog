...
5. Институт с точки зрения социальной онтологии — это параметризованное правило в равновесии; с точки зрения теории игр — это система механизмов изменения стратегического пространства игры, включающая в себя правило принятия решения (устройство корреляции) и модификатор платежа как вероятностную санкцию за отклонение от рекомендованной стратегии; математически это подкласс БКР с более жёстким, чем стандартное, условием «повиновения» (obedience) с $\delta$-параметром.
...
Следуя Бергеману и Моррису, зададим игру с неполной информацией $(G,S)$.  Базовая игра $G$ включает конечное множество игроков $i=1,\dots,I$, множества действий $A_i$, множество состояний $\Theta$ и общую априорную веру $\psi \in \Delta(\Theta)$.  Полезности игроков описываются функциями $u_i : A \times \Theta \to \mathbb{R}$, где $A = \prod_i A_i$.  Информационная структура $S = ((T_i)_i,\pi)$ задаёт множества типов $T_i$ и распределение типов $\pi : \Theta \to \Delta(T)$, $T = \prod_i T_i$.

Правило принятия решения (decision rule) — это отображение
$$
\sigma : T \times \Theta \to \Delta(A),
$$
которое каждой конфигурации типов $t\in T$ и состоянию $\theta\in\Theta$ сопоставляет распределение над профилями действий.  В духе принципа раскрытия, $\sigma$ можно интерпретировать как коррелирующее устройство, выдающее игрокам приватные рекомендации действий.

Чтобы определить БКР, Бергеман и Моррис определяют «послушность» (obedience) — условие на правила принятия $\sigma$. Решащее правило $\sigma : T \times \Theta \to \Delta(A)$ называется **послушным** для $(G,S)$, если для каждого игрока $i$, каждого типа $t_i \in T_i$ и каждого рекомендованного ему действия $a_i \in A_i$ выполняется неравенство
$$
\sum_{a_{-i},t_{-i},\theta}
u_i((a_i,a_{-i}),\theta)\,
...
Интуитивно: если медиатор при состоянии $\theta$ и профиле типов $t$ использует $\sigma$ как правило выдачи рекомендованных действий, то, зная свой тип и рекомендацию, каждому игроку выгодно следовать рекомендации, а не отклоняться. 

Для каждого игрока $i$, его типа $t_i$, рекомендуемого действия $a_i\in A_i$ и любой девиации $a_i'\in A_i$ выполняется
$$
...
\tag{OB}
$$
Ожидания в $(OB)$ берутся относительно $\psi,\pi$ и $\sigma$ в стандартном смысле Бергемана–Морриса. 

<!--[? пример про инвестора БКР]-->
Для иллюстрации рассмотрим пример, который приводят сами Бергеман и Моррис [@bergemann2016]. Представим игру с двумя состояниями мира: «плохое» (B) и «хорошее» (G), с априорной вероятностью 1/2 каждое. Есть один игрок — фирма, которая может инвестировать или не инвестировать. Выигрыши представлены в следующей матрице:

|          | Состояние B | Состояние G |
|----------|-------------|-------------|
| Инвестировать | -1          | x (0 < x < 1) |
| Не инвестировать | 0           | 0            |

В отсутствие дополнительной информации фирма не будет инвестировать, поскольку ожидаемая полезность равна (1/2)(-1) + (1/2)(x) = (x-1)/2 < 0.  Теперь предположим, что есть «информационный дизайнер» (государство, регулятор), который может выбрать информационную структуру и рекомендательную функцию σ, распределяющую сигналы игроку о состоянии мира. В терминах БКР рекомендательная функция σ: Θ → Δ(A) сопоставляет каждому состоянию мира вероятность инвестирования p_θ.  Условие повиновения для рекомендации «инвестировать» требует, чтобы ожидаемая полезность от инвестирования была не меньше, чем от отклонения (не инвестировать): 

$$\frac{1}{2} p_G \cdot x + \frac{1}{2} p_B \cdot (-1) \geq 0$$

Решая это неравенство, получаем ограничение на рекомендательную функцию: $p_G ≥ p_B + 1/x$.

Этот результат показывает несколько важных вещей. Во-первых, информационный дизайнер может создать структуру, при которой инвестиция становится равновесием — это аналог института. Во-вторых, это равновесие существует только при определённой конфигурации  информации — параметр вероятности здесь r заменён на характеристику информационной структуры. В-третьих, при неблагоприятных условиях (когда сигнал говорит «плохое состояние») рекомендация может быть невыполнена, и тогда институт «коллапсирует».  Этот пример демонстрирует центральную роль параметра r — вероятности того, что информационная структура (или санкционный механизм) «работает» в пользу соблюдения правила. Без этого параметра правило «инвестируй» не может быть реализовано как БКР. 

---

### Интерпретация в терминах института

Перенося этот пример на язык социальной онтологии, получаем следующую
картину. Институт — это пара (σ, S), где σ — рекомендательная функция
(правило), а S — информационная структура и санкционная
инфраструктура, при которых σ реализуется как БКР. Ключевой параметр —
вероятность r того, что при нарушении правила последует санкция. Эта
вероятность онтологически отражает состояние инфраструктуры
мониторинга и принуждения.

Когда r = 0 — санкционный механизм не работает — правило существует
лишь как «правило-по-форме» (в терминологии Остром), но не как
институт. Когда r > 0 — появляется возможность для БКР, и правило
начинает существовать как институт. При r → 1 — санкция почти
гарантирована — институт становится «репрессивным» в смысле Хиндрикса.

Таким образом, параметр r является *определяющим* для существования
института как БКР. Без работающего санкционного механизма (r = 0) БКР
невозможно — следовательно, нет и института. Этот вывод подтверждает
нашу онтологическую позицию: первичнее не правило, а санкционная
инфраструктура.
