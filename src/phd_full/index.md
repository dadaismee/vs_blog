...
Как наша модель объясняет возникновение и стабилизацию института? Рассмотрим модель на примере игры ЯГБ с двумя пастухами. Два пастуха встречаются на высохшем русле реки, чтобы пасти скот, и используют условную стратегию «если пришёл первым, паси скот». В динамическом смысле, если условная стратегия — ЭСС, то она вытесняет стратегии Голубя и Ястреба из популяции. Положим, что условная стратегия уже закрепилась как ЭСС. 
...
Основные параметры рассматриваемой нами игры — ценность ресурса $V$ и цена конфликта $C$. Игра повторяется: в каждом раунде попарно встречаются представители каждого из двух племён. Поскольку Буржуа — уже ЭСС, фактически в игре доступны две стратегии — Буржуа ($B$) и Блефующий ($L$). 
...
\end{array}
$$
<figcaption>Платёжная матрица ЯГБЛ со стратегией Блефа</figcaption>
</div>
...
Поэтому концептуально игру с пастухами можно разбить на два этапа: доинституциональный и институциональный. Пороговое значение $\delta$ определяет режим: если оно выше порога в $\frac{V}{4}$, то фактически игра идёт по матрице выше, а если ниже порога, то по матрице ниже:
...
В этой матрице условная стратегия $B$ защищена от редкого мутанта‑блефующего $L$, если при встрече с Буржуа она даёт строго больший выигрыш:
...
Чтобы продемонстрировать это, рассмотрим *репликаторную динамику* в упрощённой игре между Буржуа и Блефующими (без Ястребов и Голубей — потому что мы вслед за Мейнардом Смитом [@maynardsmith1982] и Скирмсом [@skyrms2014] допустим, что Буржуа стала ЭСС и вытеснила остальные стратегии из популяции).
...
Для проверки устойчивости условной стратегии Буржуа против *редкого* блефа рассмотрим стартовую долю Буржуа $x_B(0) = 0.98$ и Блефующих $x_L(0) = 0.02$. Численное решение уравнения репликатора показывает[^code]:
...
В сети игр защиту условной стратегии Буржуа определяет не $\delta$ сама по себе, а произведение $r\delta$. Большая $\delta$ при малом $r$ (формально жёсткая, но редко применяемая санкция) может давать тот же эффект, что и умеренная $\delta$ при высоком $r$ (мягкая, но почти неизбежная санкция), а слишком малое $r\delta$ вообще не обеспечивает динамической устойчивости Буржуа-стратегии.

Для популяции из двух стратегий $B$ и $L$ линия $r\delta = \frac{V}{4}$ на плоскости $(\delta,r)$ отделяет область параметров, в которой условная стратегия Буржуа динамически устойчива и $x_B^{*}(\delta,r)$ близка к 1, от области, в которой блеф внедряется в популяцию и $x_B^{*}(\delta,r)$ падает. 

![Институт как область параметров](assets/BL_heatmap_delta_r.png)

Жёлтым показана область параметров $(\delta, r)$, где условная стратегия Буржуа находится *в режиме института* — предполагает санкцию за отклонение, и является предсказуемой для агентов. Фиолетовым показана область параметров, где условная стратегия находится *в режиме прото-института* — формально санкция за отклонение от условной стратегии есть, но она недостаточно часто применяется в сети игр. В этой области редкие мутанты‑блефующие имеют преимущество против Буржуа, и потому Буржуа не может обеспечивать эволюционно устойчивый порядок. Однако это не означает, что вся популяция состоит из блефующих — институт просто не способен самостоятельно стабилизировать кооперацию и остаётся хрупким, частичным, «де юре».

ОСТАНОВИЛСЯ ЗДЕСЬ
<!--вставить картинку фазового портрета для 4 стратегий-->
...
Чем выше $N$, — чем больше игр в сети — тем больше $M$ игр с блефом и тем сложнее обеспечить достаточное значение $r\delta$ по сети игр. 
