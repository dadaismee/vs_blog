...
<!--[? что значит институт как регулятивное правило и чем это отличается от других подходов]-->

<!--- чтобы найти ответ → чекнуть статьи гуалы и грефа-->

Подход к институтам как регулятивным правилам делает акцент на регуляции поведения, а не на создаваемых статусах с деонтическими силами. 

<!--The rule-based conception belongs to a venerable tradition that goes back to the founders of modern social science. The idea of institutions as ‘rules of the game’ (Spielregeln) is already in Weber (1910: 459) and Parsons (1935)-->

Экономист Дуглас Норт — один из самых заметных представителей этого подхода — определяет институты как «правила игры» или «созданные человеком ограничения поведения» и делит их на формальные (законы, бюрократия) и неформальные (традиции, нормы, конвенции). Важно, что оба типа *принудительны* и регулируют поведение, потому что подкреплены силой. Эти «правила игры» структурируют стимулы и ограничения человеческого взаимодействия [@north1990]. 

<!--[? как выглядят теории внутри этого подхода?] -->
...
Хотя Норт не формализует эту идею, это можно было представить как изменение функции выплат:
$$
...
Наиболее полный пример подхода регулятивных правил можно найти в теории Элинор Остром [@crawford1995; @ostrom2005]. Она изучала сообщества, которые без внешнего надзора со стороны муниципалитета или государства регулируют общие ресурсы, что позволило ей сформулировать концепцию иерархии правил [@ostrom2005]. Она выделяет 4 уровня правил: операционные (индивидуальное поведение), правила коллективного выбора, конституционные, мета-конституционные. Каждый следующий уровень определяет правила выбора правил на предыдущем уровне — чтобы изменить операционные правила, нужно обсудить это на уровне правил общего выбора. К тому же, правила более высокого уровня «дороже» изменить — это видно на примере конституции. Подобная идея иерархии правил также есть у Норта. Сразу можно заметить, что подобная идея напоминает бесконечный регресс и проблемы «охраны охранников» (who guards the guardians), согласно которой гарант соблюдения правил вам нуждается в надсмотре из-за отсутствия основания для доверия.
...
<!--[? критика: почему правил недостаточно для объяснения принудительной силы института]-->

<!--[? критика: разделение содержания правил и принуждения]-->
...
Подход институтов-как-правил рассматривает принуждение к следованию правилам отдельно от содержания и процесса формирования правил [@greif2011].
...
Грейф и Кингстон считают, что несмотря на то, что правило может координировать действия агентов, в конечном счёте именно ожидаемое поведение других мотивирует собственное поведение агента, а не абстрактное правило.  
...
<!---->
---
Аргументация теории ПвР строится на недостаточности правил и равновесий по отдельности. И если правила могут существовать и не исполняться, то с равновесиями всё не так однозначно. Равновесий, по мнению Гуалы и Хиндрикса, недостаточно для объяснения социальных институтов, поскольку они схватывают слишком широкий класс феноменов — в том числе территориальные споры животных. Чтобы это показать, авторы сравнивают две игры — «Ястреб-Голубь-Буржуа» самцов бабочек и подобную игру с выпасом скота у людей.

В первой игре два самца бабочки разрешают конфликт за территорию, координируя поведение на простом физическом сигнале — «кто первым занял место» [@davies1978]. Их стратегия *условна* (conditional) и зависит от наблюдаемых признаков среды — наличия или отсутствия оппонента на территории. У каждой бабочки есть две стратегии: 
...
Во второй игре два человеческих племени — Нуэр и Линка — разделены высохшим руслом реки, которое служит для них границей [@guala2015]. 

![Граница как физический маркер (слева) и как абстрактное правило (справа). Взято из @guala2016a](assets/guala.png)

Стратегия членов каждого племени также условна, но зависит уже не от физического маркера — порядка очередности — а от разделяемого *правила* — «Если земля находится к югу (или к северу — для другого племени) от русла реки, пасти скот». 

<!--- добавить матрицу игры племен — todo-->

<!--Хотя эта стратегия связана с высохшим руслом, в ней это русло важно не само по себе, а как *средство кодирования информации*. Высохшее русло поддерживает (scaffolds) более глубокую эпистемическую структуру, основанную на общем знании (или общей причине полагать) [@lewis1969] о том, что это высохшее русло *означает*. -->

Агенты координируют действия на разделяемом убеждении — правиле, а не на физическом маркере. Это устройство корреляции, *абстрагированное* от физического маркера с помощью *когнитивной репрезентации* разделяемого правила. 

Гуала и Хиндрикс говорят, что люди, в отличие от животных, могут выбирать, на чем координировать свои действия [@guala2015], и поэтому к равновесиям нужно добавить правила.
