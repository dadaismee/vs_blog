...
Чтобы ответить на этот вопрос, мы вновь рассмотрим игру. Однако поскольку пример Гуалы с выпасом скота уже содержит в себе готовую конвенцию, он не подходит. Поэтому рассмотрим обновлённую игру с выпасом скота из прошлой секции, где два пастуха встречаются на территории высохшей реки и решают, кто из них будет пасти скот.

В этой игре один из пастухов пригоняет стадо на территорию высохшей реки раньше другого. Это служит асимметрией ролей, подобной асимметрии в примере Гуалы с бабочками: кто-то пришёл раньше, а кто-то позже, и это может повлиять на исход ситуации.

Как предполагает Гуала и как мы показали в прошлой секции, в данном случае стратегия Буржуа будет КР. Однако это так только в случае, если мы полагаем, что факт раннего прибытия — это факт природы, как в примере с бабочками. Однако там возможные состояния мира — или типы игроков — взаимоисключающие (если один владелец, то другой — захватчик). Это в очень грубой форме напоминает общее знание, поскольку каждый игрок «знает» структуру игры и возможные информационные разбиения (если говорить языком Вандершраафа).

Гуала переносит эту логику на игру с выпасом скота и руководствуется идеей, схожей с конвенцией Вандершраафа, где важна определённая информационная структура КР, а не только его базовая форма.

ОСТАНОВИЛСЯ ЗДЕСЬ

Точнее, что 

Однако в случае с пастухами это может быть не так: прибывший позже может сказать, что он был здесь ещё до первого, и первый может либо поверить и уступить, либо не поверить и вступить в схватку. 
