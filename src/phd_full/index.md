...
Чтобы равновесие было глобальным в сети игр, нужно, чтобы *санкция применялась в каждой из игр*. И если санкция $\delta$ — это механизм компенсации блефа *в одной игре,* то «доставка санкции» — это механизм *масштабирования* компенсации блефа на сеть игр.
...
Это условная вероятность применения санкции к нарушению. Она отражает *идеализированную* надёжность санкций: реальная надёжность зависит от *зафиксированных* нарушений — подмножества $M_{\text{obs}} \subseteq M$. Когда институт ещё формируется и нестабилен, фиксация — это простые формы мониторинга, сплетни и т.д., однако в действующем институте это могут быть регистрации дел, видеофиксация и т.д. Так мы получаем *эмпирическую оценку надёжности санкций*:

$$\hat r_{M_{\text{obs}}} = \Pr(\text{санкция} \mid \text{зафиксированное нарушение}) = \frac{1}{M_{\text{obs}}} \sum_{i \in M_{\text{obs}}} E_i$$
...
Надёжность санкции показывает, насколько часто санкция следует за нарушением. Это влияет на матрицу выигрышей для сети игр: становится важным усреднённое (или «эффективное») значение санкции, при котором условная стратегия снова станет ЭСС:
...
Для конкретной симуляции удобно взять $\delta_{min} = 12.51$, чтобы удовлетворить строгому неравенству $\pi(I,I) > \pi(J,I)$ в ЭСС [@maynardsmith1982]. При $\delta = 12.5$ выигрыши $B$ и $L$ в игре против Буржуа совпадают ($25$), так что условная стратегия Буржуа теряет статус ЭСС, потому что исчезает перекос в её пользу. Однако значения выше этого порога стабильно приведут Буржуа к чистой ЭСС. Чтобы продемонстрировать это, рассмотрим *репликаторную динамику* в упрощённой игре между Буржуа и Блефующими (без Ястребов и Голубей — потому что мы допустим, что Буржуа стала ЭСС и вытеснила остальные стратегии из популяции).
...
Пусть $x_i$ — доля стратегии $i$, $f_i$ — её ожидаемый выигрыш, $\bar f$ — средний выигрыш по всем стратегиям:
...
Для проверки устойчивости условной стратегии Буржуа против *редкого* блефа рассмотрим стартовую долю Буржуа и Блефующих $x_B(0) = 0.98$, $x_L(0) = 0.02$. Численное решение уравнения репликатора показывает[^code]:

[^code]: Код симуляции можно посмотреть в «Приложении 1».

![Доля стратегий Буржуа и Блефующий при $\delta = 0$](assets/BL_combined_4-0.98.png)
...
- При $\delta = 12.5$ стратегии занимают равные доли популяции, что говорит о нейтральной (слабой) устойчивости равновесия;
- При $\delta = 12.51$ доля Буржуа растёт, но не вытесняет Блефующих — для этого как минимум нужно большее временное окно;
...
Однако есть положить сеть игр, то значение имеет не только существование этого порога, но и то, как фактически реализуются санкции в этой сети.
...
Это означает, что надёжность санкции 
 снижает жёсткость санкции, нужную для поддержания условной стратегии Буржуа как ЭСС — санкция вряд ли будет непропорционально высокой. Это так, поскольку если реалистично допустить, что санкция имеет стоимость поддержания 
, то чем выше надёжность санкции, — то есть чем в большем количестве игр она применена в нынешнем раунде — тем «дешевле» её поддерживать[^sanction-cost]. В этом смысле 
 — это метапараметр нашей модели, не влияющий напрямую на глобальную стабильность Буржуа.
...
Тогда *надёжность санкции* в сети игр:
