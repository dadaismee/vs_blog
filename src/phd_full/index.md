...
Например, есть 5 повторяющихся стратегических ситуаций на пространственной сетке, в трёх из которых санкция была применена в текущем раунде: $E = \{0, 1, 1, 0, 1\}$. Тогда $r$ по ним будет $\frac{3}{5} = 0.6$, а эффективная санкция в текущем раунде $\delta_{eff} = 0.6\delta$. 

<!--- добавить картинку пространственно сетки игр — todo-->
...
Основные параметры игры, влияющие на исходы — ценность ресурса $V$ и цена схватки $C$. В игре 4 стратегии: Ястреб, Голубь, Буржуа и Блефующий. Мы сразу добавим $\delta$-параметр к платежу Блефующего. Структура матрицы выглядит следующим образом:

<div class='table'>
$$
\begin{array}{|l|c|c|c|c|} \hline 
 & \textbf{Ястреб} & \textbf{Голубь} & \textbf{Буржуа} & \textbf{Блефующий} \\ \hline 
\textbf{Ястреб} & \frac{V-C}{2} & V & \frac{3V-C}{4} & \frac{3V-C}{4} \\ \hline 
\textbf{Голубь} & 0 & \frac{V}{2} & \frac{V}{4} & \frac{V}{4} \\ \hline 
\textbf{Буржуа} & \frac{V-C}{4} & \frac{3V}{4} & \frac{V}{2}& \frac{V}{2} \\ \hline 
\textbf{Блефующий} & \frac{V-C}{4} & \frac{3V}{4} & \frac{3V}{4} - \delta & \frac{V}{2} \\ \hline 
\end{array}
$$
...
\begin{array}{|l|c|c|c|c|} \hline 
 & \textbf{Ястреб} & \textbf{Голубь} & \textbf{Буржуа} & \textbf{Блефующий} \\ \hline 
\textbf{Ястреб} & \frac{V-C}{2} & V & \frac{3V-C}{4} \\ \hline 
\textbf{Голубь} & 0 & \frac{V}{2} & \frac{V}{4} \\ \hline 
\textbf{Буржуа} & \frac{V-C}{4} & \frac{3V}{4} & \frac{V}{2} \\ \hline 
\textbf{Блефующий} & \frac{V-C}{4} & \frac{3V}{4} & \frac{3V}{4} & \frac{V}{2} \\ \hline 
\end{array}
$$
<figcaption>Платёжная матрица ЯГБ 4*4 со стратегией Блефа</figcaption>
</div>

При $\delta = \frac{V}{4} = 50/4  = 12.5$ профиль с условными стратегиями $(B, B)$ будет ЭСС и противостоит внедрению блефующей стратегии. Точнее, минимальное нужное значение — $12.51$, поскольку при $12.5$ платёж Блефующего ($25$) будет равным платежу Буржуа ($25$), чего недостаточно для ЭСС — одна стратегия должна давать строго больший выигрыш [@maynardsmith1982]. Если снизить значение $\delta$, равновесие разрушится и перейдёт к флуктуации — мерцанию между разными исходами. Наглядно показать это в долях каждой стратегии в популяции агентов можно с помощью *репликаторной динамики* при более высоких и более низких значениях $\delta$. Уравнение репликатора:

<!--вставить уравнение репликатора — todo-->
...
Такая картина идентична ситуации, где выигрыш Блефующего равен 0. При стандартных $V = 50$ и $C = 100$ это возможно при $\delta = 37.5$ и $r = 1$. Это означает, что для модели Гуалы и Хиндрикса, где институт — это КР, отклонение от координации настолько невыгодное, что в 1.5 раза превышает выигрыш условной стратегии при условии, что это является общим знанием и все знают о последствиях мискоординации. 
