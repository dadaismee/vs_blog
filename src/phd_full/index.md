...
Рассмотрим модель на примере игры ЯГБ с двумя пастухами. Два пастуха встречаются на высохшем русле реки, чтобы пасти скот, и используют условную стратегию «если пришёл первым, паси скот». В динамическом смысле, если стратегия Буржуа — ЭСС, это значит, что она вытесняет стратегии Голубя и Ястреба из популяции. Положим, что условная стратегия уже закрепилась как ЭСС. 
...
Для простоты модели мы не будем указывать конкретный источник ущерба и вероятностную структуру санкций внутрь стратегии Буржуа. Вместо этого положим, что минимальный источник санкции $\delta$ — сам Буржуа: если он не верит блефующему, он реализует санкцию и наносит ущерб. Формально источник санкции здесь не важен, но он важен онтологически: в доинституциональном состоянии она порождается непосредственной реакцией участника (Буржуа), тогда как в институциональном состоянии она доставляется через отдельный канал — полицию, суд, совет старейшин, репутационную систему и т.д. Таким образом, один и тот же параметр $\delta$ имеет различные «носители» в доинституциональном и институциональном режимах; именно это различие позволит трактовать институт как канал доставки санкций между играми.
...
Матрица 4х4 здесь не имеет онтологического смысла (как и матрица Гуалы с выпасом скота), потому что отражает *два разных этапа формирования института*. Стратегии Ястреба ($H$) и Голубя ($D$) определяют поведенческую основу для условных стратегий и важны в доинституциональном состоянии. Когда Буржуа — ЭСС, она вытесняет Ястреба и Голубя из популяции. А стратегия Блефа становится возможной только когда Буржуа-стратегия уже есть. 

Поэтому игру с пастухами логичнее разбить на два этапа: доинституциональный и институциональный. Для первого случая матрица будет стандартной, как в прошлой главе:
...
В ситуации, где Буржуа-стратегия «паси, если пришёл первым» — уже ЭСС и появилась стратегия Блефа, матрица примет следующий вид:
...
\hline 
 & \textbf{B} & \textbf{L} \\ 
\hline 
\textbf{B} & \frac{V}{2} & \frac{V}{4} \\ 
\hline 
\textbf{L} & \textcolor{red}{\frac{3V}{4} - \delta} & \frac{V}{4} \\ 
\hline 
\end{array}

<figcaption>Структура матрицы ЯГБЛ 2*2 со стратегией Блефа</figcaption>
</div>

При стандартных значениях $V = 50$, $C = 100$ это даёт:
...
В этой матрице стратегия Буржуа $B$ защищена от редкого мутанта‑блефующего $L$, если при встрече с Буржуа она даёт строго больший выигрыш:
...
Для конкретной симуляции удобно взять $\delta_{min} = 12.51$, чтобы удовлетворить строгому неравенству $\pi(I,I) > \pi(J,I)$ в ЭСС [@maynardsmith1982]. При $\delta = 12.5$ выигрыши $B$ и $L$ в игре против Буржуа совпадают ($25$), так что условная стратегия Буржуа теряет статус *чистой* ЭСС, потому что исчезает перекос в её пользу. Однако значения выше этого порога стабильно приведут Буржуа к чистой ЭСС. 
...
Однако есть положить сеть игр, то значение имеет не только существование этого порога, но и то, как фактически реализуются санкции в этой сети. Пусть есть $N$ стратегических ситуаций, где разные пастухи из племён Нуэр и Динка встречаются, чтобы пасти скот. У этих ситуаций одинаковая матрица выплат и значение $\delta$. В $M$ ситуаций имел место блеф, в $S \in M$ которых за ним последовала санкция. Все $N$ ситуаций связаны друг с другом информационным каналом, по которому передаётся информация о фактах применения санкции среди игр. 
...
Линия $r\delta = \frac{V}{4}$ на плоскости $(\delta,r)$ отделяет область параметров, в которой условная стратегия Буржуа динамически устойчива и $x_B^{*}(\delta,r)$ близка к 1, от области, в которой блеф внедряется в популяцию и $x_B^{*}(\delta,r)$ падает. 

![Институт как область параметров](assets/BL_heatmap_delta_r.png)

Жёлтым показана область параметров $(\delta, r)$, где условная стратегия Буржуа находится *в режиме института* — предполагает санкцию за отклонение, и является предсказуемой для агентов. Фиолетовым показана область параметров, где условная стратегия находится *в режиме прото-института* — формально санкция за отклонение от условной стратегии есть но она недостаточно часто применяется в сети игр. Это наводит нас на мысль, что институт — это не одно равновесие, а *конфигурация параметров, где динамическое равновесие в принципе реализуемо*.
