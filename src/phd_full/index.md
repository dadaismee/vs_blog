...
Жёлтым показана область параметров $(\delta, r)$, где условная стратегия Буржуа находится *в режиме института* — предполагает санкцию за отклонение, и является предсказуемой для агентов. Фиолетовым показана область параметров, где условная стратегия вытеснена Блефом и находится в *доинституциональном* состоянии — институт фактически коллапсирует. Линия *прототинситута* проходит по $r\delta = \frac{V}{4}$ — формально санкция за отклонение от условной стратегии есть, но она недостаточно часто применяется в сети игр, и её едва хватает для удержания *нейтральной (слабой) устойчивости*. Протоинститут не способен *стабилизировать* кооперацию и остаётся хрупким, частичным, «де юре».

В репликаторной модели с четырьмя стратегиями $H, D, B, L$ протоинститут вообще не реализуется даже как слабо устойчивое состояние динамики, как в случае с динамикой $B-L$: 
...
Вместо этого здесь реализуется *бифуркационная неустойчивость*: при любом сколь угодно малом отклонении параметров или состава популяции динамика моментально уходит либо в устойчивый режим доминирования Буржуа-стратегии (институт), либо в режим доминирования смешанной популяции Ястребов и Блефующих (коллапс института). Численно это проявляется в том, что точное пороговое значение $\delta$ невозможно «поймать» с помощью репликаторной динамики: из‑за чувствительности к округлению траектория всегда скатывается в один из двух аттракторов, а само пороговое состояние остаётся теоретической границей в пространстве параметров, а не наблюдаемым аттрактором, как случае с динамикой $B-L$.

Поэтому модель с четырьмя стратегиями больше подходит для описания *возникновения и распада института*. Она показывает, что при недостаточных санкциях система откатывается к доинституциональным режимам. Однако динамика сложившегося института устроена проще: стратегии $H$ и $D$ практически вытеснены, и релевантными остаются только условная стратегия и блеф. В этой сокращённой модели порог $r\delta = \frac{V}{4}$ задаёт границу между устойчивым институтом и его коллапсом. Протоинститут в этом смысле — не отдельное устойчивое состояние динамики, а пороговая поверхность в параметрическом пространстве санкций, которая разделяет две устойчивые фазы: институциональную и доинституциональную.
...
Другая важная идея — что надёжность санкции зависит от общего числа игр с блефом: чем выше $N$, — больше игр в сети — тем больше $M$ игр с блефом и тем сложнее обеспечить достаточное значение $r\delta$ по сети игр.

Это позволяет сказать две вещи: 

- чем больше игр в сети, тем большее значение $r\delta$ требуется для динамической стабильности института
- «объективный» и «доступный игрокам» уровень надежности $r$ могут различаться: игрокам доступно ограниченное число игр

В репликаторной модели параметр r не задан извне как “божий коэффициент”, а представляет собой свёртку двух эндогенных факторов: (i) количества контекстов и игр, где возможен блеф и санкция, и (ii) объёма вложений инвесторов в канал санкций. Увеличение охвата (больше игр) и/или инвестиций повышает r и может перевести институт из области коллапса в область устойчивости.

<!--нужен ли предел и структруный/опытный слои?-->

<!--Я трактую \delta и r как эндогенные характеристики: они порождаются стратегиями участников, инвестирующих в мониторинг и санкции. Формально это можно было бы описать коэволюционной динамикой, где \delta(t) и r(t) зависят от доли таких стратегий. Для целей настоящего аргумента достаточно зафиксировать, что институт соответствует устойчивому режиму, в котором эндогенно поддерживаемое r\delta превышает порог V/4, тогда как доинституциональные состояния остаются по эту сторону порога.-->

<!--мб сказать про строительсвт строительсвто канала, заинтересованных и популяцию инвесторов?-->

<!--В дальнейших разделах мы будем интерпретировать $r$ как результат существования или отсутствия *информационного канала*, связывающего отдельные игры, и покажем, как вложения в такой канал со стороны “элит” позволяют удерживать систему в области $r\delta > \frac{V}{4}$, где условная стратегия Буржуа остаётся эволюционно устойчивой.-->

---

институт — это информационно‑силовой канал, который повышает фактический r и позволяет описывать популяцию игр как систему с устойчиво высокой надёжностью санкций.

Формально можно рассматривать последовательность оценок $\hat r_N$, где $N$ — число игр, по которым усредняется наблюдаемая статистика санкций. При достаточно больших $N$ и при условии, что статистическая структура среды стабильна, эти оценки стремятся к некоторому пределу:

$$r^{*} = \lim_{N \to \infty} \hat r_N,$$

который отражает **эффективную надёжность** канала санкций в данной популяции игр. Этот \(r^{*}\) — не “божий” параметр, а предельная характеристика фактической практики: доля нарушений, которая в долгосрочной перспективе превращается в санкции, с учётом как невидимых нарушений, так и неполной наблюдаемости для исследователя. [ueaeco.github](https://ueaeco.github.io/working-papers/papers/ccp/CCP-13-07v2.pdf)

<!--конфликт за ресурсы онтологически первичен-->

Это позволяет аккуратно развести **структурный уровень** и **уровень опыта**. На структурном уровне мы описываем игру между ролями (например, буржуа и оппортунистами), где исходные выплаты модифицируются параметрами \(\delta\) и \(r^{*}\): \(\delta\) задаёт силу санкции, а \(r^{*}\) — предельную надёжность её доставки. Для многих моделей можно показать, что существует область значений \((\delta, r^{*})\), в которой условная стратегия буржуа является динамически устойчивой: при этих параметрах эволюционная или обучающая динамика вытесняет отклонения, и популяция закрепляется в буржуазном режиме. В таком описании институт — это не просто набор декларативных норм, а **структура игры и область параметров**, при которых определённый профиль поведения поддерживается устойчивой сетью санкций. [pmc.ncbi.nlm.nih](https://pmc.ncbi.nlm.nih.gov/articles/PMC4811019)

На уровне опыта агенты сталкиваются не с \(r^{*}\) как абстрактной величиной, а с конечными выборками и шумными индикаторами: \(\hat r_N\) рассчитывается либо по их собственному опыту, либо по публично доступным сигналам вроде слухов, судебной статистики или новостей о карательных кампаниях. При малом \(N\) и при высокой доле невидимых нарушений эти оценки могут существенно занижать истинную надёжность канала, а поведение будет реагировать именно на них. Эпистемический слой здесь — это описание того, как агенты, опираясь на ограниченные и систематически цензурированные данные о нарушениях, **приближаются** (или не приближаются) к реальному структурному режиму \((\delta, r^{*})\). Это хорошо согласуется с фаллибилистской интуицией: участники могут заблуждаться относительно того, насколько силён и стабилен институт, и только постепенно корректируют свои ожидания по мере накопления опыта. [yalelawjournal](https://yalelawjournal.org/article/agencies-as-litigation-gatekeepers)

Если перейти к более реалистичной картине, становится понятно, что и \(\delta\), и \(r\) в долгосрочном режиме должны рассматриваться как **эндогенные** характеристики институциональной конфигурации, а не как внешние параметры. В реальных системах \(\delta\) (сила санкции) зависит от того, какие ресурсы общество или конкретные группы готовы вкладывать в наказание: от бюджета правоохранительных органов, длительности и суровости наказаний, технологий мониторинга и социальной стигматизации. Аналогично истинное \(r\) (доля нарушений, доходящих до санкции) определяется плотностью мониторинга, вероятностью доносов, качеством расследований, а также стратегическими решениями о приоритизации дел. То, что мы видим в данных — это лишь вершина айсберга, а реальное \(r\) формируется через практики наблюдения, фильтрации и отбора дел для преследования. [nature](https://www.nature.com/articles/srep17050)

Чтобы концептуально зафиксировать эту эндогенность, полезно ввести в модель **третью популяцию** — инвесторов в институт (институциональных предпринимателей). Тогда в популяции игр взаимодействуют:  

1) обычные агенты, выбирающие между буржуазным поведением и нарушением;  
2) оппортунисты/нарушители, оценивающие риск и цену санкций;  
3) инвесторы, которые решают, сколько ресурсов направить на мониторинг, расследование и наказание.  

В такой трипопуляционной динамике \(\delta(t)\) и \(r(t)\) становятся функциями от стратегий и долей инвесторов: при увеличении инвестиций в мониторинг и санкции усиливается как **сила** (\(\delta\)), так и **надёжность** (\(r\)) канала, причём сами инвесторы получают выгоды от стабильного буржуазного режима (например, через налоговые доходы, ренту от защищённой собственности или политический контроль). Институт в этом смысле — это **устойчивый режим совместных инвестиций**, при котором поддержание достаточно высокой \(\delta^{*}\) и \(r^{*}\) становится выгодным для значимой части акторов, а не внешний механизм, навязанный извне. [economics.unibocconi](https://economics.unibocconi.eu/sites/default/files/files/media/attachments/eei20140402100850.pdf)

---

Рассмотрим случай с несколькими играми. Стратегии и матрица выигрышей остаются прежними, добавляется лишь параметр $r$ — надёжность санкции в популяции игр. 

<!--- описать функцию про предел и показать график пространства параметров todo-->

Можно заметить, что отношение $r$ и $\delta$ можно выразить как функцию: 
$$f(s) = $$
...
Это означает, что *надёжность санкции $r$ снижает жёсткость санкции, нужную для поддержания условной стратегии Буржуа как ЭСС* — санкция вряд ли будет непропорционально высокой. Это так, поскольку если реалистично допустить, что санкция имеет стоимость поддержания $k(\delta)$, то чем выше надёжность санкции, — то есть чем в большем количестве игр она применена в нынешнем раунде — тем «дешевле» её поддерживать[^sanction-cost]. В этом смысле $k(\delta)$ — это метапараметр нашей модели, не влияющий *напрямую* на глобальную стабильность Буржуа.

<!--[^sanction-cost]: добавить про стоимость санкции из phd - todo-->

При этом, если убрать из модели возможность блефа, то санкция станет не нужна, и мы увидим картину, близкую к описанной Гуалой и Хиндриксом в их теории ПвР: условная стратегия Буржуа будет ЭСС (и КР, если рассмотреть её статически). Интересно, что такая картина идентична ситуации, где выигрыш Блефующего равен 0. При стандартных $V = 50$ и $C = 100$ это возможно при $\delta = 37.5$ и $r = 1$. Это означает, что  для модели Гуалы и Хиндрикса, где институт — это чистая координация, выраженная как КР, отклонение от координации настолько невыгодное, что в 1.5 раза превышает выигрыш условной стратегии при условии, что это является общим знанием и все знают о последствиях мискоординации.
...
<!--- вставить репликаторную динамику игры без блефа — todo-->

<!--- чекнуть ии-текст про эпистемический r — todo-->

---

В до сих пор обсуждаемой идеализированной модели r вводился как параметр усреднённой игры — доля эпизодов, в которых санкция за блеф фактически реализуется. В более реалистичной картине r не дан заранее, а оценивается агентами по конечному числу игр. Пусть E_t\in\{0,1\} — индикатор применения санкции в игре G_t, а \hat r_N = \frac{1}{N}\sum_{t=1}^N E_t — эмпирическая оценка надёжности санкций на основе N наблюдённых эпизодов. Тогда ожидаемый выигрыш Блефующего против Буржуа принимает вид
u(L,B;\delta,\hat r_N) = 37.5 - \hat r_N \delta.
При N\to\infty \hat r_N сходится к “истинной” вероятности санкции p, и мы возвращаемся к ранее введённому параметру r=p. При конечном и малом N, однако, агенты опираются на шумную оценку \hat r_N; в этом случае даже при параметрах (\delta,p), лежащих в институциональной области p\delta > \frac{V}{4}, система может не попасть в режим x_B^{*}(\delta,r)\approx 1, просто потому что в её “истории” пока слишком мало эпизодов санкционирования.

---


<!--- сказать что параметры — эндогенные, но задаются как внешние для параметрического описания института todo-->


## 3.4 Границы модели
Стоит рассмотреть другие игры помимо ЯГБ, чтобы проверить валидность нашей модели и обнаружить границы её применимости для объяснения социальных институтов и их онтологической унификации.

Основное требование нашей модели — *асимметрия ролей игроков*, которую оба из них могут наблюдать и оценивать: больше/меньше, раньше/позже и т.д. Согласно Скирмсу и Херрману, динамическое создание конвенций (то есть, условных стратегий) требует наличия *любой* интерпретируемой асимметрии [@herrmann2021]. Это означает, что модель схватывает ситуации, где есть наблюдаемая асимметрия ролей.

<!--- привести пример наблюдаемая асимметрия ролей — todo-->
<!--- сказать в каких играх возможна асимметрия ролей — todo-->
