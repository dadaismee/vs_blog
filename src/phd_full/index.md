...
где «владелец» получает $V$ с вероятностью ¹⁄₂, «захватчик» получает $0$ с вероятностью ¹⁄₂. Показываем, что Блефующий использует неполную информацию о сигнале, всегда сигнализируя «я — владелец» и в остальном играя как Буржуа:

$$\pi(M,B) = \frac12 \cdot \frac{V}{2} + \frac12 \cdot V = \frac{3V}{4}$$

Выигрыш Блефующего больше выигрыша Буржуа: $\frac{3V}{4} > \frac{V}{2}$ при $C > V$.

Стратегия Блефующего направлена в первую очередь на эксплуатацию Буржуа, что подрывает статус последней как ЭСС. Это означает, что условная стратегия сама по себе не может быть эволюционно стабильной: *корреляции стратегий недостаточно для стабильных социальных контрактов* — вероятность блефа модифицирует платежи и нарушает равновесие.

Восстановление стабильности требует поддерживающего механизма. Как мы показали в прошлой главе, это некоторый $\delta$-параметр, который выравнивает платежи стратегий Буржуа и Блефующего:

$$\pi(M, B) - \delta < \pi(B, B)$$

Подобный дельта-параметр можно вывести, определив платежи стратегий Буржуа и блефующего чужака. Вычтем из выплаты блефующего чужака штраф $\delta$, применяемый к факту эксплуатации неопределенности, а не к самому конфликту:
$$
\pi_{M}^\delta=V-\delta
$$
Это означает, что *эволюционно стабильное равновесие в условных стратегиях не может существовать в среде с ненулевой вероятностью блефа без модификатора платежей*. Введение $\delta$ делает стратегию Буржуа устойчивой на уровне платежа. Получаем минимально необходимый модификатор платежа для поддержания коррелированной (условной и симметричной) стратегии Буржуа для агента:
...
Сейчас мы покажем, что институт с модификатором платежа $\delta$ стабилен только *локально*, в одной игре, но рушится при наличии множества игр. Это позволит формально отделить институт от протоинститута.

Локальная ЭСС означает устойчивость стратегии в одной игре $𝐺_i$. Глобальная устойчивость — в популяции, где много экземпляров игры $G_N$ происходят в разных местах и моментах. Это пространственная множественность или временная повторяемость:
