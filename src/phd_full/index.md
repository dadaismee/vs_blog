...
Предположим, что пастухи обоих племён следуют правил «пришёл первым — пасти скот, а если вторым — уступить», но игроки могут от него отклоняться. Роли «первого» и «второго» задаются устройством корреляции — очерёдностью прибытия.

По правилу собственности «второй» должен играть $D$, а отклонением является выбор $H$. Пусть с вероятностью $p$ «первый» фактически ведёт себя «мягко» и играет $D$, а с вероятностью $1-p$ следует правилу и играет $H$. Тогда ожидаемый выигрыш захватчика при следовании правилу (играть $D$):
$$
\mathbb E[u(D)] = (1-p)\cdot 0 + p\cdot 25 = 25p;
$$
А при нарушении правила (играть $H$):
$$
\mathbb E[u(H)] = (1-p)\cdot (-25) + p\cdot 50 = -25 + 75p.
$$
Премия за нарушение правила для «второго»:
$$
\Delta u(p) = \mathbb E[u(H)] - \mathbb E[u(D)] = (-25 + 75p) - 25p = -25 + 50p.
$$
Если $p > 0{,}5$, то $\Delta u(p) > 0$: когда «первые» достаточно часто ведут себя мягко, «второму» выгодно нарушать правило и играть ястреба. При $p \le 0{,}5$ правило собственности поддерживается структурой игры и среды, и премии за отклонение нет. 

Теперь предположим, что нарушение правила влечёт санкцию $\delta > 0$ с эффективностью $r \in [0,1]$. Тогда ожидаемый выигрыш «второго» при нарушении становится:
$$
\mathbb E[u(H)]_{\text{инст}} = -25 + 75p - r\delta.
$$
Институт «собственности» реализован, если следовать правилу (играть $D$ в роли захватчика) не менее выгодно, чем его нарушать:

$$
\mathbb E[u(D)] \;\ge\; \mathbb E[u(H)]_{\text{инст}}.
$$

Подставляя выражения, получаем:

$$
25p \;\ge\; -25 + 75p - r\delta
\quad\Longleftrightarrow\quad
r\delta \;\ge\; -25 + 50p = \Delta u(p).
$$

где $\Delta u(p) > 0.5$ — структурный стимул «второго» к нарушению правила в мире, где «первые» с вероятностью $p$ ведут себя мягко.

Онтологически $p$ описывает состояние фоновой координации: насколько часто игроки и без санкций готовы следовать правилу «ястреб, если первый и голубь, если второй». Чем выше $p$, тем меньше премия за отклонение $\Delta u(p)$ и, соответственно, меньше минимальное требование к $r\delta$. Институт же фиксируется не самим $p$, а неравенством $r\delta \ge \Delta u(p)$: санкционный слой должен перекрывать структурный стимул к девиации, задаваемый игрой и коррелирующим устройством.

<!--- поменять на 4-картинки в одной todo-->
![](assets/delta_r_alpha_0.51_with_line.png)

<!--- расписать про разницу при разной вероятности todo-->
- при $\delta = 6$ достаточно $r \ge 1{,}2/6 = 0{,}2$;
- при $\delta = 3$ необходимо $r \ge 1{,}2/3 = 0{,}4$;
- при $\delta = 1$ требовалось бы $r \ge 1{,}2$, что невозможно, и при столь низкой суровости санкций институт не может удерживать правило. 
...
В онтологическом плане $r\delta$ здесь — институциональный слой (право, санкции общины, репутационные потери и т.п.), который компенсирует «сырую» премию $\Delta u(\alpha)$ и делает отклонение от правила невыгодным. При $r\delta < \Delta u(\alpha)$ правило остаётся лишь «бумажной нормой» или «спящим» институтом, тогда как при $r\delta \ge \Delta u(\alpha)$ оно реализуется как *строгое КР*.
...
Чтобы это приллюстрировать, рассмотрим другой случай — игру «Охота на оленя», где каждый выбирает либо охотиться на оленя $S$ и получить больший выигрыш, либо единолично охотиться на зайца $H$ и получить меньший выигрыш.Пусть матрица игры имеет следующий вид:
...
Устройство корреляции рекомендует сигнал $s = S$ с вероятностью $p$:

- при следовании сигналу (играть $S$):
$$\mathbb E[u(S)] = 4p + 0\cdot(1-p) = 4p;$$

- при отклонении (играть $H$):
$$\mathbb E[u(H)] = 3p + 2(1-p) = 2 + p.$$

Премия за отклонение:

$$\Delta u(p) = \mathbb E[u(H)] - \mathbb E[u(S)] = (2+p) - 4p = 2 - 3p.$$

Если представить ситуацию «идеального института», где все ожидают, что оппонент играет $S$ с вероятностью 1 (то есть $p = 1$), то:
...
$$\Delta u(1/2) = 2 - 3\cdot \frac12 = 2 - 1.5 = 0.5.$$
...
$$
График для этой игры будет следующим:
<!--- сделать график для охоты на оленя todo-->

Иначе говоря, «граница института» в «Охоте на оленя» не фиксируется на $1/4$. Для классической матрицы с \((4,0,3,2)\):

- если мы считаем, что институт “пытается удержать” координацию на $S$ при $p<1$, то при $p=1/2$ пороговая санкция — $r\delta = 0.5$;  

- при других $p$ порог будет $2-3p$.

В нашем численном примере $\Delta u = 0.5$, и, следовательно, институт существует только в той области параметров $(r,\delta)$, где произведение ожидаемой силы санкций и их эффективности не меньше $0.5$. Численно это означает, что:

<!--дописать численно и добавить графики про охоту на оленя todo-->
...
**Определение 1.** Пара $(\mu,(\sigma_i)_i)$ называется институциональным коррелированным равновесием, если для каждого игрока $i$, каждого сигнала $s_i \in S_i$ с $\mu(s_i)>0$ и любого альтернативного действия $a_i' \in A_i$ выполняется
...
\;\ge\;
\mathbb E\bigl[
u_i(a_i',\sigma_{-i}(s_{-i})) - r\delta \cdot D_i(s_i,a_i')
\mid s_i
\bigr].$$

Если $(r,\delta)$ малы и $r\delta < \Delta u_i$ для некоторых сигналов и отклонений, то правило, пусть даже оно организовано как конвенция, не будет институционально устойчивым: блеф остаётся выгодным, и институциональный режим не реализуется. Поэтому институт в предложенном смысле — это не любая игра с произвольной $\delta$, а определённый **режим работы конвенционального медиатора и санкционного канала**, существующий только в определённой области параметров $(r,\delta)$.

### 3.3 Эмпирический смысл надёжности санкции
