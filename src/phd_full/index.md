...
$$
которое каждой конфигурации типов $t\in T$ и состоянию $\theta\in\Theta$ сопоставляет распределение над профилями действий. Можно трактовать $\sigma$ как коррелирующее устройство, выдающее игрокам приватные рекомендации действий. Это правило выбирает *дизайнер информации*, как называют это Бергеман и Моррис.
...
3. Приватн реализуется тип каждого игрока $t_i$. Типом Бергеман и Моррис называют любую приватную информацию агента;
4. Игроки получают приватные сообщения от устройства корреляции согласно правилу $\sigma$;
5. Игроки выбирают действия согласно их априорной информации и полученным сообщениям;
...
Дополнительно авторы уточняют, что есть выгода от координации — если оба инвестора вкладываются, то получают небольшой дополнительный выигрыш за счёт «стратегических комплементарностей» — эффекта масштаба, репутации, координации и другого. Тогда для каждого инвестора важны не только его ожидания относительно «качества» проекта, но и ожидания относительно того, что сделает другой.

У каждого инвестора есть *собственный приватный сигнал* о состоянии экономики. Например, каждый получает сигнал «хорошие новости» или «плохие новости» про проект. Если проект действительно потенциально прибыльный, то «хорошие новости» приходят чаще, чем «плохие». А если проект «плохой», то, наоборот, «плохие новости» более вероятны.

То есть сигналы не идеальны, но по ним можно приблизительно «угадывать» состояние. Как только инвестор видит свой сигнал, он вычисляет для себя апостериорную вероятность того, что экономика сейчас в хорошем или плохом состоянии. Именно эти *апостериорные верования* и определяют, насколько для него оправдан риск инвестировать.

Например, инвестор с сигналом «хорошие новости» верит, что вероятность хорошего состояния велика, и поэтому склонен инвестировать, а инвестор с сигналом «плохие новости» считает, что вероятность плохого состояния высока, и его интуитивная реакция — не инвестировать.

Информационный дизайнер может наблюдать реальное состояние проектов и, возможно, приватные сигналы инвесторов, а затем посылает каждому инвестору рекомендацию: «инвестируй» или «не инвестируй». Его цель может быть, например, максимизировать вероятность того, что инвесторы вкладываются (поддержать инвестиционную активность), но при этом он вынужден учитывать, что инвесторы рациональны: они будут следовать рекомендациям только если это действительно в их интересах с точки зрения их апостериорных верований.

Это и есть содержательный смысл условия «повиновения» — если инвестор с определённым сигналом получает рекомендацию «инвестируй», то при своих апостериорных верованиях он должен считать инвестицию для себя не хуже отклонения от рекомендации. А если он получает рекомендацию «не инвестируй», эта рекомендация тоже не должна идти вразрез с его апостериорными оценками.
...
Несмотря на свою абстрактность и унифицирующий пафос, БКР не способно «схватить» сущность социального института само по себе, поскольку даёт лишь очень общее и «тонкое» описание равновесия, зависимого от информационной структуры как минимум по трём линиям. 

Во‑первых, хотя БКР говорит о «повиновении», это не принуждение в более строгом смысле. Любой устойчивый паттерн поведения, поддерживаемый ожиданиями и информацией, попадает в класс БКР: это может быть координационная конвенция, случайное фокальное равновесие или результат информационного манипулирования. Ничто в самой концепции БКР не различает рациональный наилучший ответ и распознвание правила и отклонения от него.

Во‑вторых, БКР не различает институциональные и другие последствия. В функции выигрыша любой элемент смешан с технологическими и другими эффектами: сломалась машина, упала цена или игрок начал испытывать стыд. Всё это — числа в матрице, обеспечивающие устойчивость. Для онтологии института, напротив, нужно выделить те компоненты, которые являются социально‑созданными ответами на соблюдение или нарушение правила.

В‑третьих, без дополнительной параметризации БКР не даёт удобного способа говорить о «силе» одного и того же института. Оно либо есть, либо нет. Между тем, именно градуальность — сильные и слабые институты, жёсткие и мягкие нормы и санкции — выходит на первый план [@ostrom1990; @bicchieri2005; @hindriks2022].
...
Здесь $u_i(a',\theta = 1)$ — базовая функция выигрышей, заданная структурой игры. Тогда стандартное условие повиновения (obedience) для правила $\sigma$ в терминах БКР переписывается без изменения формы, но с новой функцией выигрышей.

Онтологически это означает, что мы не меняем саму структуру БКР: санкция $\delta$ просто становится частью выигрыша, но теперь этот выигрыш явно зависит от того, в каком «институциональном мире» находятся игроки и нарушают ли они правило.

Если зафиксировать все параметры, кроме $\psi(\theta = 1)$ и $\delta$, то можно переписать условие *институционального* повиновения как 

$$
\psi(\theta = 1)\,\delta \;\ge\; \Delta u_i,
$$

где $\Delta u_i = \mathbb{E}[\tilde u_i(a_i')] - \mathbb{E}[\tilde u_i(a_i)]$ — структурная премия за девиацию без учёта санкций. Это не «универсальная формула БКР», а удобная репараметризация именно того фрагмента ожидаемой полезности, который связан с санкцией и институциональным принуждением. Она позволяет обсуждать отдельно два онтологически важных аспекта санкционного механизма:
...
БКР как концепт решения хорош своей богатой параметризацией — в частности, он позволяет отделять *априорные и апостериорные вероятности* и трактовать их как объективную частоту и субъективную оценку срабатывания санкционного механизма при фиксации остальных параметров. В конечном счёте, решающее значение в реализации БКР имеет именно апостериорная оценка игроков.

Если частоту реализации мира с санкциями и ход Природы мы уже затрагивали выше, то апостериорные оценки агентов по поводу наблюдаемых сигналов о работе санкционного механизма — нет. Субъективная вероятность санкции при девиации $Pr(\theta = 1 | t_1)$ — апостериорная оценка игрока, возникающая внутри условных ожиданий. Она зависит от вероятности попасть в мир с санкциями $\psi$, информационной структуры $\pi$ и решающего правила $\sigma$.

Онтологически это означает, что институт — не только материальный механизм фиксации нарушений и реализации санкций с параметрами $(\psi(\theta = 1),\delta)$, но и объект верований: агенты могут ошибаться относительно действительной частоты применения санкции, что выражается в апостериорной оценке $Pr(\theta | t_i)$. Один и тот же институт (то есть поведенческий паттерн) может быть устойчив или хрупок в зависимости от того, насколько поддерживающая его санкционная инфраструктура выглядит работающей в глазах агентов. Тогда свёрнутое условие повиновения принимает форму
...
Таким образом, предварительно можно сказать, что институт — это правило, реализованное как Байес‑скоррелированное равновесие в игре, где функция выигрышей содержит выделенный санкционный компонент $\delta >0$, а условие послушания имеет вид $\psi(\theta = 1)\delta \geq \Delta$. Это позволяет, во‑первых, отличать такие институты от конвенций — равновесий без санкций, а во‑вторых, параметризовать их устойчивость по осям надёжности и жёсткости санкционного механизма.

Таким образом, в первом приближении, институт — это БКР с более жёстким, чем стандартное, условием повиновения, в которое включена санкция за отклонение от рекомендуемой стратегии. Онтологически институт это параметризованное правило в равновесие с механизмом фиксации нарушении и реализации санкций.

Тогда информационная структура БКР задаёт, как разные типы агентов оценивают вероятность санкционного исхода (через апостериорные верования по состояниям), параметр $\delta$ выражает интенсивность самих санкций или институциональных последствий, а правило совместно с санкцией определяют, какие действия считаются соблюдением, а какие — нарушением.

По нашему мнению, институт в духе Гуалы и Хиндрикса возникает там, где эти три элемента сходятся: правило включено в игру, санкционная компонента $\delta$ делает соблюдение устойчивым, а информационная структура формирует такие верования, при которых условие obedience выполняется для соответствующих стратегий. Без выделения санкции как особого параметра БКР остаётся просто формальным описанием устойчивой координации. С ним же мы получаем тот «утолщённый» объект, с которым можно уже сравнивать социальные институты в гуаловском смысле: системы правил, поддерживаемые устойчивыми ожиданиями и специфическими социальными последствиями соблюдения и нарушения.
...
<!--В данной игре с неполной информацией $(G,S)$ под **институтом** я буду понимать норму $\sigma : T \times \Theta \to \Delta(A)$, реализуемую как Bayes‑коррелированное равновесие игры с санкциями: существует модифицированная система полезностей $\tilde u_i$, в которой отклонение от предписанного действия приводит к потере полезности в санкционных состояниях, такая, что $\sigma$ послушно для $(G,\tilde u,S)$ в смысле условия obedience выше. -->

<!--В этой рамке и *объективная* частота санкционных состояний (заданная $\psi(\theta)$), и *субъективные* вероятности $\Pr(\theta\mid t_i,a_i)$ входят в одни и те же ожидания в условии obedience; эффективная сила санкций задаётся не отдельным параметром, а совместным вкладом $\tilde u_i$, $\psi$ и постериоров в выполнение этих неравенств. -->


<!--[? наш пример с ястреб-голубем и графиками]-->
## 3.2 Проблема силы института и возможное байесовское решение
Ранее мы сказали, что БКР позволяет выделять объективную и субъективную оценку применения санкций при нарушении правила. Формально обе их них содержатся в условии повиновения, однако мы можем развести их, чтобы делать онтологически значимые различения. Чтобы это показать, рассмотрим редуцированную 2×2‑версию игры Ястреб-Голубь.
...
Это позволяет различить ситуацию, где фактически обеспеченный санкционный механизм слабее, чем вера агентов в него. Чтобы не вдаваться в тяжеловесные байесовские вычисления и продемонстрировать нашу идею о расхождении реальной и воспринимаемой надёжности санкций, положим, что субъективная оценка систематически выше реальной частоты (например, $r = 1.5\psi$). В таком случае институт как БКР будет устойчив даже при деградировавшей или нестабильной санкционной инфраструктуре.

![$psi(\theta)$ и субъективная оценка агента $r = Pr(\theta = 1 | t_i)$ вместе показывают «запас прочности» института](assets/protoinstitute.png){width=70%}

Это означает, что агенты *переоценивают* вероятность санкций за отклонение от правила. Голубая пунктирная линия на графике показывает субъективно воспринимаемый порог стабильности института. В зоне под ней институт в глазах агентов коллапсирует, а над ней — работает, даже несмотря на то, что объективной инфраструктуры для этого уже недостаточно. Жёлтая область между линиями — институт как *ожидаемый порядок*, а не как *самоподдерживающееся равновесие*: равновесие здесь есть, но оно хрупкое и зависит исключительно от верований агентов, которые могут измениться, в результате чего институт коллапсирует и передвинется в красную зону графика. 

Чтобы институт функционировал именно как самоподдерживающееся равновесие в строгом смысле, реальные и субъективные уровни вероятности санкций должны лежать в одной и той же «зелёной» области: ожидания игроков не должны систематически расходиться с фактической частотой применения санкций $r\delta \approx \psi\delta$.

В пределе полной информации и устойчивых ожиданий «сильный институт», находящийся в «зелёной зоне» в нашей модели совпадает с конвенцией Вандершраафа (или институтом Гуалы), реализованную в виде коррелированного равновесия.
...
Таким образом, наша модель развивает и частично обобщает хиндриксовскую метрику силы институтов. Она сохраняет его базовую интуицию — сила как избыток мотивации по отношению к структурной премии, но переписывает её в байесовских терминах. Это позволяет связать теорию институтов с Байес‑коррелированным равновесием и информационным дизайном и различать разные режимы одного и того же института (правила) в зависимости от расхождения между объективной и субъективной вероятностями санкций, чего нет в явном виде в его формальной схеме.
...
Институт — это параметризованное правило в равновесии, имеющее *санкционный механизм*, который надстраивается над заданным правилом «следуй сигналу» и базовой игрой и систематически изменяет выигрыши нарушителей на величину $\delta>0$ с вероятностью $r\in[0,1]$.
