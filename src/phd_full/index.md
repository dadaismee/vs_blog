...
<!---->
<!--Однако для динамической онтологии института это не проблема.-->
...
### 3.2. Проблема масштаба: параметр $r$ и надёжность компенсации блефа

<!--- сверить описание модели с 3.3 — todo-->
...
Локальная ЭСС означает устойчивость стратегии в одной игре $𝐺_i$. Глобальная устойчивость — в популяции, где много экземпляров игры $G_N$ происходят в разных местах (и/или моментах):

$$\mathcal{G} = \{G_1, G_2,…,G_N\}$$

Важно, что хотя $\delta$ — объективный параметр стабильности равновесия, его применение в каждой конкретной игре *не гарантировано*. Санкция может не реализоваться по ряду причин, что отменяет дисконтирование платежа Блефующего, в результате чего он получит платёж $\frac{3V}{4}$ вместо $\frac{3V}{4} - \delta$.

Для поддержания глобального равновесия нужен механизм, который позволил бы агентам поступать так, *как если бы санкция была применена мгновенно в любой из игр популяции*. И если санкция $\delta$ — это механизм компенсации блефа *в одной игре,* то «доставка санкции» — это механизм компенсации шума между играми при *масштабирования* санкции $\delta$ на несколько игр.

Бинарная случайная величина $E ∈ \{0, 1\}$ определяет, применена ли санкция в конкретной игре: $1$ означает, что санкция $\delta$ применена, а $0$ — не применена. Платежи в локальной игре содержат скрытый параметр $r \coloneq P(E = 1)$ — объективную вероятность применения санкции. Она означает *надёжность механизма принуждения* (enforcement), а при рассмотрении популяции игр — частоту реализации санкции в популяции игр. Поскольку в одной локальной игре применение санкции гарантировано (по результатам прошлой главы), то $r = 1$, поэтому этот параметр скрыт. Однако для популяции игр *эффективная санкция*, при которой условная стратегия снова станет ЭСС, будет:

$$\delta_{eff} = r \times \delta$$

В популяции игр $r$ — это частота реализации санкции: 

$$r = \frac{1}{N} \sum^N_{i=1}E_i$$

Например, есть 5 повторяющихся стратегических ситуаций, в трёх из которых санкция была применена в текущем раунде: $E = \{0, 1, 1, 0, 1\}$. Тогда $r$ по ним будет $\frac{3}{5} = 0.6$, а эффективная санкция в текущем раунде $\delta_{eff} = 0.6\delta$. 
...
Это значение можно назвать *порогом институциональности* поведенческого паттерна: ниже этого значения применение санкций станет непредсказуемым.

Повышение надежности санкции — частоты ее применения в играх популяции — требует механизма *повышения связности* между играми — статистической зависимости фактов реализации санкции. Это можно выразить как снижение дисперсии $D(r)$, энтропии доставки санкции $H(r)$ или повышение *взаимной информации* между двумя случайными играми $I(E_i, E_j) > 0$. Подобный механизм выравнивает среднее значение санкции $\delta_{eff}$ в популяции игр. Статистическая связь между реализациями санкций в играх популяции — это структура информационного канала между играми, а не модификация платежа или отдельная сущность. 

Если надёжность применения санкции в популяции — статистический факт, то *механизм повышения этой надёжности — онтологически важный аспект института*. Такой механизм делает применение санкции после эксплуатации неопределённости Блефуюшим предсказуемым среди игр популяции. Подобный механизм и составляет *глубинную онтологическую основу института*, что мы подробно рассмотрим в Главе 4. Иначе говоря, для восстановления *глобального* равновесия, необходимо, чтобы реализация санкции а одной игре была *информативной* относительно других и предсказуемой на уровне системы. Именно идея о *системности санкции* позволит нам дальше связать равновесия, регулятивные и конститутивные правила.


## 3.3 Пример использования модели
<!--[? можете привести пример работы модели?]-->

<!--- интегрировать ии-текст — todo-->

Рассмотрим модель на примере игры ЯГБ с двумя пастухами. Два пастуха встречаются на высохшем русле реки, чтобы пасти скот, и используют условную стратегию «если пришёл первым, паси скот». Предположим, что эта условная стратегия уже закрепилась как ЭСС. Один из пастухов племени Нуэр, вновь приведя стадо и увидев, что пастух племени Динка уже на месте, решает схитрить: заявляет, что пришёл первым, но был вынужден отвести стадо из‑за волков. Пастух Динка может либо поверить и уступить, либо не поверить и наказать его — забрать несколько голов скота, испортить репутацию, создать угрозу будущего исключения и т.д. Мы будем обозначать суммарный ущерб от такого наказания через $\delta \geq \frac{V}{4}$ при $V = 50$ и $C = 100$.

Для простоты модели мы не будем “вшивать” конкретный источник ущерба и вероятностную структуру санкций внутрь стратегии Буржуа. Вместо этого положим, что минимальный источник санкции $\delta$ — сам Буржуа: если он не верит блефующему, он реализует санкцию и наносит ущерб. Формально источник санкции здесь не важен, но он важен онтологически: в доинституциональном состоянии она порождается непосредственной реакцией участника (Буржуа), тогда как в институциональном состоянии она доставляется через отдельный канал — полицию, суд, совет старейшин, репутационную систему и т.д. Таким образом, один и тот же параметр $\delta$ имеет различные «носители» в доинституциональном и институциональном режимах; именно это различие позволит трактовать институт как канал доставки санкций между играми в Главе 4.

Основные параметры игры — ценность ресурса $V$ и цена конфликта $C$. Игра повторяется: в каждом раунде попарно встречаются представители каждого из двух племён. Всего доступны четыре стратегии: Ястреб (H), Голубь (D), Буржуа (B) и Блефующий (L). Мы сразу добавим параметр $\delta$ в выплату Блефующего. Матрица выигрышей имеет вид:
...
Для конкретной симуляции модели удобно взять $\delta_{min} = 12.51$, чтобы удовлетворить строгому неравенству $\pi(I,I) > \pi(J,I)$ в ЭСС [@maynardsmith1982]. При $\delta = 12.5$ выигрыши $B$ и $L$ в игре против Буржуа совпадают ($25$), так что условная стратегия Буржуа теряет статус ЭСС, потому что исчезает перекос в её пользу. Если снизить значение $\delta$, равновесие разрушится и перейдёт к флуктуации — мерцанию между разными исходами. 

Стоит подчеркнуть, что $\delta > \frac{V}{4}$ гарантирует Буржуа как чистую ЭСС только локально, против редкого мутанта $L$ в одной игре. В смешанных популяциях, где доли стратегий эволюционируют, этого порога недостаточно для защиты от блефа. Чтобы увидеть это, рассмотрим *репликаторную динамику* в упрощённой двухстратегической игре между Буржуа и Блефующими.

Пусть $x_i$ — доля стратегии $i$, $f_i$ — её ожидаемый выигрыш, $\bar f$ — средний выигрыш:
...
Для проверки устойчивости условной стратегии Буржуа против редкого блефа рассмотрим стартовую долю Буржуа и Блефующих $x_B(0) = 0.98$, $x_L(0) = 0.02$. Численное решение уравнения репликатора показывает:

<!--- заменить графики на один 4-фигурный — todo-->

При $\delta = 0$ условная стратегия Буржуа не ЭСС, и блеф захватывает популяцию:

![Доля стратегий Буржуа и Блефующий при $\delta = 0$](assets/BL_delta-zero.png)

При $\delta = \frac{V}{4} = 12.5$ условная стратегия Буржуа перестанет быть ЭСС, и популяция перейдёт в смешанное равновесие $B-L$:

![Доля стратегий Буржуа и Блефующий при $\delta = \frac{V}{4} = 12.5$](assets/BL_delta-equal.png)

При $\delta = 12.51$ Буржуа начинает вытеснять Блефующего, но его доля остаётся ненулевой, и возникает смешанное ЭСС по типам:

![Доля стратегий Буржуа и Блефующий при $\delta > \frac{V}{4} = 12.51$](assets/BL_delta-min.png)

Минимальное значение, при котором популяция перейдёт к Буржуа-стратегии как чистой ЭСС$\delta_{min} = 12.58$:

![Доля стратегий Буржуа и Блефующий при $\delta > \frac{V}{4} = 12.58$](assets/BL_delta-high-full.png)

Выше этого значения уже при любом размере санкции — например, в 1.5 раза превышающем платёж Буржуа, как у Гуалы и Хиндрикса ($\delta = 37.5$) — Буржуа также закрепится как чистая ЭСС:

![Доля стратегий Буржуа и Блефующий при $\delta >> \frac{V}{4} = 37.5$](assets/BL_delta-high.png)

Чтобы удостовериться в роли граничного значения $\delta = \frac{V}{4}$, проверим динамику при равном составе популяции $x_B(0) = x_L(0) = 0.5$:

![Доля стратегий Буржуа и Блефующий при равном составе популяции](assets/BL_combined_4.png)

Порог \(\delta > \frac{V}{4}\) действительно защищает Буржуа в локальном смысле ЭСС, служа аттрактором в игре с двуся стратегиями, но для популяции, важно не только существование этого порога, но и то, как фактические санкции распределены в сети взаимодействий.

Иначе говоря, локальная ЭСС в одной игре требует в нашем примере как минимум $\frac{V}{4} + 0.01$. Однако этого недостаточно, если допустить несколько связанных копий одной игры.

В приближенных к реальности условиях агенты не играют одну и ту же игру с заданным значением санкции. Вместо этого они участвуют в множестве взаимодействий, в одних из которых нарушение наказывается, а в других — нет. Обозначим через $G_1,\dots,G_T$ отдельные игры с одинаковой структурой выплат и значением $\delta$, но с различной реализацией санкции. 

ОСТАНОВИЛСЯ ЗДЕСЬ

Пусть в игре $G_t$ “реальная” санкция за блеф против Буржуа равна $\delta_t$. Для простоты можно считать, что $\delta_t$ принимает два значения: либо $\delta_t = \delta$ (санкция применена), либо $\delta_t = 0$ (санкция не применена). Введём индикатор $E_t \in \{0,1\}$ применения санкции в игре $G_t$, так что $\delta_t = E_t \cdot \delta$.

Тогда надёжность санкции в популяции игр можно выразить как

\[
r = \mathbb{E}[E_t],
\]

то есть как долю игр, в которых санкция фактически реализуется. Если стратегии игроков формируют ожидания относительно применения санкций, то Блефующий оценивает свою выплату против Буржуа в “усреднённой игре” с учётом этого r:

\[
u(L,B;\delta,r) = 37.5 - r\delta.
\]

Условная стратегия Буржуа будет защищена от редкого блефа в такой усреднённой игре, если

\[
f(B,B;\delta,r) > f(L,B;\delta,r),
\]

то есть

\[
25 > 37.5 - r\delta \;\;\Longleftrightarrow\;\; r\delta > \frac{V}{4}.
\]

Таким образом, в популяции игр защиту условной стратегии Буржуа определяет не \(\delta\) сама по себе, а произведение \(r\delta\). Большая \(\delta\) при малом r (формально жёсткая, но редко применяемая санкция) может давать тот же эффект, что и умеренная \(\delta\) при высоком r (мягкая, но почти неизбежная санкция), а слишком малое \(r\delta\) вообще не обеспечивает устойчивости Буржуа. В этом смысле институт — это не только “размер наказания”, но и канал, который доставляет санкции между играми и обеспечивает достаточно высокое r.

На уровне репликаторной динамики в усреднённой игре можно ввести явно зависящий от \((\delta,r)\) выигрыш Блефующего и, как раньше, рассматривать эволюцию долей \(x_B(t;\delta,r)\) и \(x_L(t;\delta,r)\). Для каждой пары \((\delta,r)\) репликаторная динамика даёт предел

\[
x_B^{*}(\delta,r) = \lim_{t\to\infty} x_B(t;\delta,r),
\]

который описывает долю Буржуа в долгосрочном состоянии (если предел существует). Линия

\[
r\delta = \frac{V}{4}
\]

на плоскости \((\delta,r)\) отделяет область параметров, в которой условная стратегия Буржуа устойчива (и \(x_B^{*}(\delta,r)\) близка к 1), от области, в которой блеф внедряется в популяцию (и \(x_B^{*}(\delta,r)\) падает). В дальнейших разделах мы будем интерпретировать r как результат существования или отсутствия институционального канала, связывающего отдельные игры, и покажем, как вложения в такой канал со стороны “элит” позволяют удерживать систему в области \(r\delta > \frac{V}{4}\), где условная стратегия Буржуа остаётся эволюционно устойчивой.

---

Рассмотрим модель на примере нашей игры ЯГБ с двумя пастухами. В ней два пастуха встречаются на территории высохшей реки, чтобы пасти свой скот, и у них есть условная стратегия «если пришёл первым, то паси скот». Мы допустим, что она является ЭСС и что один из членов племени Нуэр, вновь приведя своё стадо на пастбище и увидев, что туда уже пришёл пастух племени Динка со своим стадом, решил схитрить и сказать ему, что на самом деле он пришёл первым, но был вынужден отвести стадо в сторону, потому что рядом проходила стая волков. Пастух Динка может либо поверить в это и уступить, либо не поверить и нанести небольшой ущерб ($\delta \geq \frac{V}{4}$ при $V = 50$ и $C = 100$) — забрать несколько голов скота, рассказать другим о неблагочестивом пастухе (аналог дисконтирования будущего) и т.д.

Для простоты модели мы не будем «вшивать» конкретный источник ущерба и сопутствующую ему вероятностную структуру внутрь стратегии Буржуа, а лишь скажем, что самый простой источник санкции $\delta$ — это сам Буржуа, который либо верит Блефующему и уступает, либо не верит и наказывает Блефующего. Источник $\delta$, хоть и не так важен формально, важен онтологически, поскольку он не может быть институциональным, как минимум при рассмотрении генезиса института. Другими словами, источник $\delta$ меняется при переходе из доинституционального состояния в институциональное. Подробнее об этом мы скажем в Главе 4.

Основные параметры игры, влияющие на исходы — ценность ресурса $V$ и цена схватки $C$. Это повторяющаяся игра, где попарно встречаются разные представители двух племён. Всего в игре 4 стратегии, доступных игрокам для выбора: Ястреб, Голубь, Буржуа и Блефующий. Мы сразу добавим $\delta$-параметр к платежу Блефующего. Структура матрицы выглядит следующим образом:
...
Рассчитаем платежи в матрице для стандартных значений $V = 50$ и $C = 100$: 
...
При $\delta > \frac{V}{4} = 50/4 + 0.01  = 12.51$ профиль с условными стратегиями $(B, B)$ будет локальной ЭСС и противостоит внедрению блефующей стратегии. Минимальное нужное значение — именно $12.51$, поскольку при $12.5$ платёж Блефующего ($25$) будет равным платежу Буржуа ($25$), чего недостаточно для ЭСС — одна стратегия должна давать строго больший выигрыш: $f(I, I) > f(J, I)$ [@maynardsmith1982]. Если снизить значение $\delta$, равновесие разрушится и перейдёт к флуктуации — мерцанию между разными исходами. 

Стоит подчеркнуть, что $\delta \geq 12.51$ в нашем случае защищает популяцию, состояющую из Буржуа, от *редкого мутанта* со стратегией блефа, но не гарантирует защиту в смешанной популяции. Подобное значение подходит для *статической проверки* ЭСС. В более реалистичных — динамических — условиях этого будет недостаточно. Наглядно показать это можно с помощью *репликаторной динамики* — для более высоких и более низких значениях $\delta$.
...
где $\bar{f} = \sum_j x_j f_j$ — средний ожидаемый выигрыш по популяции. 

Поскольку мы хотим проверить шансы редкого Блефующего в популяции Буржуа, в симуляции вычислим доли популяции стратегий $B$ и $L$ в игре всего с двумя стратегиями. Стартовая доля Буржуа в популяции — $x_B_0 = 0.98$, а доля Блефующих — $x_L_0 = 0.2$.
...
При $\delta = 12.51$ Буржуа вновь начнёт укрепляться, но пока ещё не займёт максимальную долю популяции. Это будет смешанная ЭСС:
...
Иначе говоря, локальная ЭСС в одной игре требует в нашем примере как минимум $\frac{V}{4} + 0.01$. Однако этого значения будет недостаточно для популяции игр.

<!--- ввести усредненные игры — todo-->

ОСТАНОВИЛСЯ ЗДЕСЬ

Сеть игр — это множество эпизодов с идентичной структурой, связанные общим информационным каналом

Есть класс однородных эпизодов (N игр с одной и той же структурой).

По этой сети эпизодов мы эмпирически считаем r как долю случаев применения санкции при нарушении (при желании — взвешенную, с оговоркой про статус/вес узлов).

В пределе большого N это даёт r*, которое просто подставляется в матрицу как компактное описание сложившегося режима санкций.

Для конечного N величины r_N и \delta_N представляют собой эмпирические частоты и средние по конечному числу связанных эпизодов. Увеличение N (числа игр, соединённых общим каналом наблюдения) уменьшает дисперсию этих оценок и тем самым приближает поведение системы к динамике с “эффективными” параметрами r,\delta. Институт можно описать как режим, в котором для данного конечного N значения r_N,\delta_N достаточно стабильны, чтобы поддерживать буржуазную стратегию как эволюционно устойчивую.

В матрице \delta и r не описывают “угаданные” или “не угаданные” санкции в отдельной игре, а суммарный статистический эффект сети фиксации и наказания блефа по популяции игр. Эту сеть — дороги, коммуникации, нормы наблюдения и отчётности — мы и называем институтом; r и \delta — его эмпирические характеристики, а не отдельные онтологические сущности.

В этой картине институт — это социоматериальная сеть доставки санкций: конфигурация инфраструктур, ролей и норм, которая делает наказания статистически надёжными. Параметры r и \delta не являются самостоятельными онтологическими объектами; они лишь компактно описывают структурные свойства этой сети. Такой подход совместим со структурным реализмом: реально существуют не “параметры”, а структуры отношений, которые мы описываем параметрически.
...
Введём популяцию игр одного типа и рассмотрим канал, по которому факты нарушений и санкций из этих игр становятся наблюдаемыми для других участников. Тогда параметр r не есть самостоятельный механизм, а статистический факт об этой популяции: идеальная доля нарушений, за которыми фактически последовала санкция. В реальности мы видим только часть нарушений (выявленные дела, расследования и т.п.), поэтому эмпирическая оценка r по N наблюдаемым играм даёт лишь нижнюю границу истинной надёжности канала санкций.

В этой рамке институт — это не сам параметр r, а механизм его повышения: социоматериальная и организационная конфигурация (каналы сообщения, мониторинг, архивы, формы, дорога к суду и т.п.), которая делает фиксацию нарушений и реализацию санкций более вероятной. Говоря кратко, институт — это информационно‑силовой канал, который повышает фактический r и позволяет описывать популяцию игр как систему с устойчиво высокой надёжностью санкций.

Рассмотрим не одну абстрактную игру, а **популяцию игр**, в которой одни и те же агенты многократно взаимодействуют в повторяющихся ситуациях с возможными нарушениями и санкциями. Для каждой игры можно, в принципе, определить “идеальный” показатель надёжности канала санкций \(r\) как долю всех реальных нарушений, за которыми фактически последовало наказание. В реальных данных мы никогда не видим полный набор нарушений; доступны лишь косвенные индикаторы вроде зарегистрированных дел, расследований, судебных решений и т.п. Поэтому эмпирическая оценка \( \hat r_N \), построенная по \(N\) наблюдаемым случаям, сама является **нижней границей** истинной надёжности канала: мы знаем, сколько нарушений было выявлено и наказано, но не знаем, сколько нарушений вообще имело место. [sciencedirect](https://www.sciencedirect.com/science/article/abs/pii/S1574073007010055)

Формально можно рассматривать последовательность оценок \(\hat r_N\), где \(N\) — число игр (или периодов), по которым усредняется наблюдаемая статистика санкций. При достаточно больших \(N\) и при условии, что статистическая структура среды стабильна, эти оценки стремятся к некоторому пределу:
\[
...
Чтобы Буржуа была глобальной ЭСС в популяции игр, нужна либо высокая санкция $\delta = C = 100$ при минимальном уровне надёжности $r = 0.1251$, либо наоборот — $r = 1$ и $\delta = 12.51$. При $\delta = \frac{V}{4} + 0.01 = 12.51$ и $r \leq 0.99$ профиль условных стратегий Буржуа перестаёт быть ЭСС по усреднённой игре, и блеф внедряется в популяцию. 

Например, если хотя бы в одной игре из 40 минимальная санкция не применяется к нарушению или если максимальная санкция применяется только в 12.5% игр популяции, Буржуа перестаёт быть ЭСС и равновесие рушится. Показать это динамически также можно с помощью репликаторной динамики. 

Стратегии и матрица выигрышей остаются прежними, добавляется лишь параметр $r$ — надёжность санкции в популяции игр. 
...
При этом, если убрать из модели возможность блефа, то $\delta$ станет не нужна и мы увидим картину, близкую к описанной Гуалой и Хиндриксом: условная стратегия Буржуа будет ЭСС (и КР, если рассмотреть её статически). Это особенно хорошо видно на репликаторной динамике. Однако картина сохраняется только в случае, если стоимость конфликта больше ценности ресурса:
...
Такая картина идентична ситуации, где выигрыш Блефующего равен 0. При стандартных $V = 50$ и $C = 100$ это возможно при $\delta = 37.5$ и $r = 1$. Это означает, что  для модели Гуалы и Хиндрикса, где институт — это чистая координация, выраженная как КР, отклонение от координации настолько невыгодное, что в 1.5 раза превышает выигрыш условной стратегии при условии, что это является общим знанием и все знают о последствиях мискоординации.
